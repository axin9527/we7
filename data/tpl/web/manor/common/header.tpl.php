<?php defined('IN_IA') or exit('Access Denied');?><?php (!empty($this) && $this instanceof WeModuleSite || 0) ? (include $this->template('common/header-base', TEMPLATE_INCLUDEPATH)) : (include template('common/header-base', TEMPLATE_INCLUDEPATH));?>
	...
	<div class="navbar navbar-inverse navbar-static-top height60 animated fadeInDown" role="navigation" style="position:static;">
		<div class="container-fluid height60">
			<ul class="nav navbar-nav height60">
				<li class="height60"><a class="height60" href="./?refresh"><i class="fa fa-reply-all"></i>返回系统</a></li>

				<?php  global $top_nav;?>
				<?php  if(is_array($top_nav)) { foreach($top_nav as $nav) { ?>
					<?php  if(!empty($_W['isfounder']) || empty($_W['setting']['permurls']['sections']) || in_array($nav['name'], $_W['setting']['permurls']['sections'])) { ?><li<?php  if(FRAME == $nav['name']) { ?> class="active height60"<?php  } else { ?> class="height60"<?php  } ?>><a class="height60" href="<?php  echo url('home/welcome/' . $nav['name']);?>"><i class="<?php  echo $nav['append_title'];?>"></i><?php  echo $nav['title'];?></a></li><?php  } ?>
				<?php  } } ?>
	...
	<div class="container-fluid">
		...
		<div class="row">
			...
			<?php  if(!empty($frames)) { ?>
				<div class="col-xs-12 col-sm-3 col-lg-2 big-menu animated fadeInLeft" data-wow-duration="2s" data-wow-delay="5s" >
					<div id="search-menu">
				...
				<div class="col-xs-12 col-sm-9 col-lg-10 animated fadeInRight" data-wow-duration="2s" data-wow-delay="5s">
					<?php  if(CRUMBS_NAV == 1) { ?>
