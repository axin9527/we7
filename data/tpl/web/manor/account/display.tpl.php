	...
	<div class="clearfix" style="margin-bottom:5em;">
		<?php  if(!$_W['isfounder']) { ?>
			<div class="alert alert-warning">
                ...
                <?php  } else if($profile['nickname']) { ?>
                <?php  echo $profile['nickname'];?>
                <?php  } else { ?>
                <?php  echo $_W['username'];?>
                <?php  } ?>
            </span>,欢迎您登陆唐盛庄园后台系统<br/><?php  if($stat['group_name']) { ?>您所在的会员组: <span class="text-strong"><?php  echo $stat['group_name'];?></span><?php  } ?>
                <?php  if($_W['user']['starttime'] > 0 && $_W['user']['endtime'] > 0) { ?>
                账号有效期限：<span class="text-strong"><?php echo  $_W['user']['starttime'] > 0 ? date('Y-m-d', $_W['user']['starttime']) : ''?><?php  if($_W['user']['starttime'] >0 && $_W['user']['endtime'] >$_W['user']['starttime']) { ?> ~~ <?php  if(empty($_W['user']['endtime'])) { ?>无限制<?php  } else { ?><?php  echo date('Y-m-d', $_W['user']['endtime'])?><?php  } ?><?php  } else { ?>无期限<?php  } ?></span><br>
                <?php  } ?>
                <?php  if($stat['maxaccount'] > 0) { ?>
                可创建 <span class="text-strong"><?php echo $stat['maxaccount'] ? $stat['maxaccount'] : 0;?> </span>个公众号，已创建<span class="text-strong"> <?php  echo $stat['uniacid_num'];?> </span>个，还可创建 <span class="text-strong"><?php  echo $stat['uniacid_limit'];?> </span>个公众号。更多商城内容请每日查看导航公告<br>
                <?php  } ?>
        ...
        <br>
        <?php  } ?>
        <?php  if($_W['isfounder'] || $stat['maxaccount'] > 0) { ?>
					...
					<ul class="panel-body list-group active_hove" data-toggle="tooltip" data-placement="top" title="如需管理该公众号,点击前往该公众号微信管理后台,即可管理微商城,活动等内容">
						...
						<li class="row list-group-item" style="line-height:60px;cursor: pointer;" data-url="<?php  echo url('account/switch', array('uniacid' => $uni['uniacid']))?>" onclick="relocal(this)">
							...
							<div class="col-xs-12 col-sm-12 col-md-4 col-lg-5 item" style="font-size:16px;">
                                <a href="<?php  echo url('account/switch', array('uniacid' => $uni['uniacid']))?>" target="_blank" class="manage"><?php  echo $account['name'];?></a>
							</div>
