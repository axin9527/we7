...
<div class="form-group">
    ...
    <div class="col-sm-9 col-xs-12">
         <?php if( ce('shop.goods' ,$item) ) { ?>
        <input type="text" name="sub_title" id="sub_title" class="form-control" value="<?php  echo $item['sub_title'];?>" />
        <span class='help-block'>最多十一个字</span>
            <?php  } else { ?>
        <div class='form-control-static'><?php  echo $item['sub_title'];?></div>
        <?php  } ?>
    </div>
</div>
<div class="form-group">
    ...
    <div class="col-sm-8 col-xs-12">
        ...
        <?php  } else if($shopset['catlevel']==2) { ?>
            <?php if( ce('shop.goods' ,$item) ) { ?>
...
<div class="form-group">
    ...
    <div class="col-sm-9 col-xs-12" >
        ...
        <label for="ispackage" class="checkbox-inline">
            <input type="checkbox" name="ispackage" value="1" id="ispackage" <?php  if($item['ispackage'] == 1) { ?>checked="true"<?php  } ?> /> 套餐
        </label>
        <label for="ishot" class="checkbox-inline">
            <input type="checkbox" name="ishot" value="1" id="ishot" <?php  if($item['ishot'] == 1) { ?>checked="true"<?php  } ?> /> 爆款热卖
        </label>
        ...
        <label for="istime" class="checkbox-inline">
            <input type="checkbox" name="istime" value="1" id="istime" <?php  if($item['istime'] == 1) { ?>checked="true"<?php  } ?> /> 限时卖(预售)
        </label>
        ...
        <label for="is_full_cat_power" class="checkbox-inline">
            <input type="checkbox" name="is_full_cat_power" value="1" id="is_full_cat_power" <?php  if($item['is_full_cat_power'] == 1) { ?>checked="true"<?php  } ?> /> 不参与满额包邮
        </label>
        <label for="is_full_money" class="checkbox-inline">
            <input type="checkbox" name="is_full_money" value="1" id="is_full_money" <?php  if($item['is_full_money'] == 1) { ?>checked="true"<?php  } ?> /> 不参与满减
        </label>
        <label for="is_full_gift" class="checkbox-inline">
            <input type="checkbox" name="is_full_gift" value="1" id="is_full_gift" <?php  if($item['is_full_gift'] == 1) { ?>checked="true"<?php  } ?> /> 不参与满赠
        </label>
          <?php  } else { ?> <div class='form-control-static'>
              <?php  if($item['isnew']==1) { ?>新品; <?php  } ?>
              <?php  if($item['ishot']==1) { ?>爆款热卖; <?php  } ?>
              <?php  if($item['isrecommand']==1) { ?>推荐; <?php  } ?>
              <?php  if($item['isdiscount']==1) { ?>促销; <?php  } ?>
              <?php  if($item['issendfree']==1) { ?>包邮; <?php  } ?>
              <?php  if($item['istime']==1) { ?>限时卖(预售); <?php  } ?>
              <?php  if($item['isnodiscount']==1) { ?>不参与折扣; <?php  } ?>
              <?php  if($item['is_full_cat_power']==1) { ?>不参满额包邮; <?php  } ?>
              <?php  if($item['is_full_money']==1) { ?>不参满减; <?php  } ?>
              <?php  if($item['is_full_gift']==1) { ?>不参满赠; <?php  } ?>
          </div>
          <?php  } ?>
...
<div class="col-sm-6 col-xs-6">
    ...
    <div style="width: auto; float: left; margin-left: 10px;"  id="type_dispatch">
        <select class="form-control tpl-category-parent" id="dispatchid" name="dispatchid">
            ...
            <option value="<?php  echo $dispatch_item['id'];?>" <?php  if($item['dispatchid'] == $dispatch_item['id']) { ?>selected="true"<?php  } ?>><?php  echo $dispatch_item['sendcity'];?><?php  echo $dispatch_item['dispatchname'];?></option>
            <?php  } } ?>
        </select>
    </div>
...
<div class="form-group">
    ...
    <div class="col-sm-9 col-xs-12">
        <div id="areas_value" class="form-control-static"><?php  echo $item['express_area'];?></div>
        <a class='btn btn-default' href="javascript:;" onclick='selectAreas1()'><span class="fa fa-plus"></span><?php  if($item['id']) { ?>修改配送区域 <?php  } else { ?>选择配送区域<?php  } ?></a>
        <input type="hidden" class='selectedareas' name="express_area" value="<?php  echo $item['express_area'];?>" />
        <span class='help-block show_h'>此处默认可以为空,为空不选择即全国都可配送,如果该商品指定在某个区域进行配送,请选择此处配送区域</span>
    </div>
</div>
<?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('web/shop/goods/selectareas', TEMPLATE_INCLUDEPATH)) : (include template('web/shop/goods/selectareas', TEMPLATE_INCLUDEPATH));?>
<script>
    if(<?php  echo $item['id'];?>) {
