...
if($operation == 'addtype'){
    ...
    ca('tmessage.view');
    $list = pdo_fetchall('SELECT * FROM ' . tablename('manor_shop_member_message_template') . ' WHERE uniacid=:uniacid order by id asc', array(':uniacid' => $_W['uniacid']));
    ...
    if(!empty($_GPC['id'])){
        $list = pdo_fetch('SELECT * FROM ' . tablename('manor_shop_member_message_template') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $_GPC['id'], ':uniacid' => $_W['uniacid']));
    ...
    if($_W['ispost']){
        ...
        if(empty($id)){
            pdo_insert('manor_shop_member_message_template', $insert);
            $id = pdo_insertid();
        }else{
            pdo_update('manor_shop_member_message_template', $insert, array('id' => $id));
        }
    ...
    ca('tmessage.delete');
    $id = intval($_GPC['id']);
    pdo_delete('manor_shop_member_message_template', array('id' => $id, 'uniacid' => $_W['uniacid']));
    message('删除成功！', $this -> createPluginWebUrl('tmessage'), 'success');
}elseif($operation == 'send'){
    ca('tmessage.send');
    $id = intval($_GPC['id']);
    $send = pdo_fetch('SELECT * FROM ' . tablename('manor_shop_member_message_template') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
    if(empty($send)){
    ...
    $value1 = $_GPC['value1'];
    $tpid1 = $_GPC['tpid'];
    pdo_update('manor_shop_member_message_template', array('sendtimes' => $send['sendtimes'] + 1), array('id' => $id));
    if($class1 == 1){
        ...
        $member = pdo_fetchall("SELECT openid FROM " . tablename('manor_shop_member') . " WHERE uniacid = '{$_W['uniacid']}'" . $where, array(), 'openid');
        if(!empty($value1)){
        ...
        $member = pdo_fetchall("SELECT openid FROM " . tablename('manor_shop_member') . " WHERE uniacid = '{$_W['uniacid']}' and isagent=1 and status=1 " . $where, array(), 'openid');
        if(!empty($value1)){
    ...
    $id = intval($_GPC['id']);
    $template = pdo_fetch('SELECT * FROM ' . tablename('manor_shop_member_message_template') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
    if(empty($template)){
    ...
    pdo_update('manor_shop_member_message_template', array('sendcount' => $template['sendcount'] + 1), array('id' => $id));
