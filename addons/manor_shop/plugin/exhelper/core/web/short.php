...
if ($operation == 'display') {
	...
	if (!empty($_GPC['shorttitle'])) {
		...
		foreach ($_GPC['shorttitle'] as $id => $shorttitle) {
			pdo_update('manor_shop_goods', array('shorttitle' => $shorttitle), array('id' => $id, 'uniacid' => $_W['uniacid']));
		}
	...
	if (!empty($total)) {
		$sql = 'SELECT id,title,thumb,shorttitle FROM ' . tablename('manor_shop_goods') . $condition . ' ORDER BY `status` DESC, `displayorder` DESC,
