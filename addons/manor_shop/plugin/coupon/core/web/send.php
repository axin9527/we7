...
if (!empty($id)){
    $coupon = pdo_fetch('SELECT * FROM ' . tablename('manor_shop_coupon') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
    if (empty($coupon)){
...
if (checksubmit('submit')){
    ...
    $members = pdo_fetchall('select id,openid,nickname,avatar from ' . tablename('manor_shop_member') . ' where openid in (' . implode(',', $mopenids) . ") and uniacid={$_W['uniacid']}");
    if (empty($members)){
        message('未找到发送的会员!', '', 'error');
    }
    if ($coupon['total'] != -1){
    ...
    pdo_update('manor_shop_coupon', $upgrade, array('id' => $coupon['id']));
    ...
    foreach ($members as $m){
        for ($i = 1;$i <= $send_total;$i++){
            ...
            if($coupon['coupontype'] == 2) {
                //劵码已使用随机劵码
                $_code = pdo_fetch('select id from '.tablename('manor_shop_coupon_gen').' where uniacid=:uniacid and coupon_id=:coupon_id and is_used=0 limit 1', array(':uniacid'=>$_W['uniacid'], ':coupon_id'=>$coupon['id']));
                pdo_update('manor_shop_coupon_gen', array('is_used'=>1, 'used_time'=>time(),'user_id'=>$m['id'], 'nickname'=>$m['nickname'], 'avastar'=>$m['avatar']), array('id'=>$_code['id']));
            }
        }
