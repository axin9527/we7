...
$sql = 'SELECT d.*, c.coupontype,c.couponname,m.nickname,m.avatar,m.realname,m.mobile FROM ' . tablename('manor_shop_coupon_data') . ' d ' . ' left join ' . tablename('manor_shop_coupon') . ' c on d.couponid = c.id ' . ' left join ' . tablename('manor_shop_member') . ' m on m.openid = d.openid and m.uniacid = d.uniacid ' . " where  1 and {$condition} ORDER BY gettime DESC";
...
$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('manor_shop_coupon_data') . ' d ' . ' left join ' . tablename('manor_shop_coupon') . ' c on d.couponid = c.id ' . ' left join ' . tablename('manor_shop_member') . ' m on m.openid = d.openid and m.uniacid = d.uniacid ' . "where 1 and {$condition}", $params);
