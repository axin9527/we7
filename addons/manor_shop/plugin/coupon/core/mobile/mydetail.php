...
$id = intval($_GPC['id']);
$data = pdo_fetch('select * from ' . tablename('manor_shop_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
if (empty($data)){
    if (empty($coupon)){
...
$num = pdo_fetchcolumn('select ifnull(count(*),0) from ' . tablename('manor_shop_coupon_data') . ' where couponid=:couponid and openid=:openid and uniacid=:uniacid and used=0 ' , array(':couponid' => $coupon['id'], ':openid' => $openid, ':uniacid' => $_W['uniacid']));
