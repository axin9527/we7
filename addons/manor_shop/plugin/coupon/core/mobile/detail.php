...
$id = intval($_GPC['id']);
$coupon = pdo_fetch('select * from ' . tablename('manor_shop_coupon') . ' where id=:id and uniacid=:uniacid  limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
if (empty($coupon)){
    if ($_W['isajax']){
        show_json(-1, '未找到优惠券');
    }
...
if ($op == 'display'){
    ...
    if ($needpay){
        ...
        pdo_update('manor_shop_coupon_log', array('paytype' => $useweixin ? 1 : 0), array('id' => $logid));
        if ($useweixin){
    ...
    $c = array_merge(array('url' => $result['url'], 'coupontype' => $coupon['coupontype']),$coupon);
    show_json(1, $c);
}
