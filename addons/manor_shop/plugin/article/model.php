...
if (!class_exists('ArticleModel')) {
	class ArticleModel extends PluginModel
	{
		public function doShare($article, $shareid, $myid)
		{
			...
			$my_click = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('manor_shop_article_share') . " WHERE aid=:aid and click_user=:click_user and uniacid=:uniacid ", array('
				:aid' => $article['id'], 
				':click_user' => $myid, 
				':uniacid' => $_W['uniacid']
			));
			...
			$articlecount = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("manor_shop_article_share") . " WHERE aid=:aid and share_user=:share_user and click_user=:click_user and uniacid=:uniacid ", array(
                ":aid" => $article["id"],
                ":share_user" => $shareid,
                ":click_user" => $myid,
                ":uniacid" => $_W["uniacid"]
            ...
            $articlecount2 = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("manor_shop_article_share") . " WHERE aid=:aid and share_user=:share_user and uniacid=:uniacid ", array(
                ":aid" => $article["id"],
                ":share_user" => $shareid,
                ":uniacid" => $_W["uniacid"]
            ));
			if ($articlecount2 >= $article["article_rule_allnum"]) {
                ...
                $articlecount3 = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("manor_shop_article_share") . " WHERE aid=:aid and share_user=:share_user and click_date>:day_start and click_date<:day_end and uniacid=:uniacid ", array(
                    ":aid" => $article["id"],
                    ":share_user" => $shareid,
                    ":day_start" => $starttime,
                    ":day_end" => $endtime,
			...
			$articlecount4 = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("manor_shop_article_share") . " WHERE aid=:aid and share_user=:click_user and click_user=:share_user and uniacid=:uniacid ", array(
                ":aid" => $article["id"],
                ":share_user" => $shareid,
                ":click_user" => $myid,
                ":uniacid" => $_W["uniacid"]
			...
			if ($article["article_rule_credittotal"] > 0 || $article["article_rule_moneytotal"] > 0) {
                $ruletotal = 0;
                $rulemoey = 0;
                $clickusercount = pdo_fetchcolumn("select count(distinct click_user) from " . tablename("manor_shop_article_share") . " where aid=:aid and uniacid=:uniacid limit 1", array(
                    ":aid" => $article["id"],
                    ":uniacid" => $_W["uniacid"]
                ));
                $sharecount = pdo_fetchcolumn("select count(*) from " . tablename("manor_shop_article_share") . " where aid=:aid and uniacid=:uniacid limit 1", array(
                    ":aid" => $article["id"],
                    ":uniacid" => $_W["uniacid"]
                ));
            ...
            pdo_insert("manor_shop_article_share", $articlearray);
            if ($articlecredit > 0) {
            ...
            if ($articlecredit > 0 || $articlemoney > 0) {
                $shopsysdata = pdo_fetch("SELECT * FROM " . tablename("manor_shop_article_sys") . " WHERE uniacid=:uniacid limit 1 ", array(
                    ":uniacid" => $_W["uniacid"]
                ));
                $articleurl = $_W["siteroot"] . "app/index.php?i=" . $_W["uniacid"] . "&c=entry&m=manor_shop&do=member";
                $articlemsg = '';
                if ($articlecredit > 0) {
        ...
        function href_replace($hreftext) {
            global $_GPC;
            $midnewtext = $hreftext;
            if (strexists($hreftext, "manor_shop") && !strexists($hreftext, "&mid")) {
		...
		function href_replace($lnk)
		{
			global $_GPC;
			$newlnk = $lnk;
			if (strexists($lnk, 'manor_shop') && !strexists($lnk, '&mid')) {
				if (strexists($lnk, '?')) {
