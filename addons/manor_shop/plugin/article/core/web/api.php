...
if ($_W["isajax"] && $_W["ispost"]) {
    if ($apido == "delarticle") {
        ...
        if (!empty($article)) {
            pdo_delete("manor_shop_article", array(
                "id" => $aid
            ));
            ...
            $article_log = pdo_fetchall("SELECT * FROM " . tablename("manor_shop_article_log") . " WHERE aid=:aid and uniacid=:uniacid ", array(
                ":aid" => $article["id"],
                ":uniacid" => $_W["uniacid"]
            ));
            if (!empty($article_log)) {
                pdo_delete("manor_shop_article_log", array(
                    "aid" => $article["id"]
                ));
            }
            $article_share = pdo_fetchall("SELECT * FROM " . tablename("manor_shop_article_share") . " WHERE aid=:aid and uniacid=:uniacid ", array(
                ":aid" => $article["id"],
                ":uniacid" => $_W["uniacid"]
            ));
            if (!empty($article_log)) {
                pdo_delete("manor_shop_article_share", array(
                    "aid" => $article["id"]
                ));
            }
        ...
        $cid      = intval($_GPC["cid"]);
        $category = pdo_fetch("SELECT * FROM " . tablename("manor_shop_article_category") . " WHERE id=:cid and uniacid=:uniacid limit 1 ", array(
            ":cid" => $cid,
            ":uniacid" => $_W["uniacid"]
        ));
        if (!empty($category)) {
            $articles = pdo_fetchall("SELECT * FROM " . tablename("manor_shop_article") . " WHERE article_category=:cid and uniacid=:uniacid ", array(
                ":cid" => $cid,
                ":uniacid" => $_W["uniacid"]
            ));
            if (!empty($articles)) {
                ...
                pdo_delete("manor_shop_article_category", array(
                    "id" => $cid
                ));
                die(json_encode(array(
                    "result" => "success"
        ...
        $cname = ($_GPC["cname"]);
        if (!empty($cname)) {
            $cates = pdo_fetch("SELECT * FROM " . tablename("manor_shop_article_category") . " WHERE category_name=:cname and id<>:cid and uniacid=:uniacid limit 1 ", array(
                ":cid" => $cid,
                ":cname" => $cname,
                ":uniacid" => $_W["uniacid"]
            ));
            ...
            if (empty($cid)) {
                ...
                pdo_insert("manor_shop_article_category", $arr);
                ...
                pdo_update("manor_shop_article_category", $arr, array(
                    "id" => $cid
                ));
                die(json_encode(array(
                    "result" => "success-edit",
        ...
        if (is_array($data)) {
            ...
            if (!empty($arr["id"])) {
                $articlekeyword = pdo_fetchcolumn("SELECT article_keyword FROM " . tablename("manor_shop_article") . " WHERE id=:aid and uniacid=:uniacid limit 1 ", array(
                    ":aid" => $arr["id"],
                    ":uniacid" => $_W["uniacid"]
                ));
            ...
            if (empty($arr["id"])) {
                ...
                ca("article.page.add");
                pdo_insert("manor_shop_article", $arr);
                $aid  = pdo_insertid();
                $desc = "insert";
            } else {
                ca("article.page.edit");
                pdo_update("manor_shop_article", $arr, array(
                    "id" => $arr["id"]
                ));
                $aid  = $arr["id"];
                $desc = "update";
            ...
            $rule = pdo_fetch("select * from " . tablename("rule") . " where uniacid=:uniacid and module=:module and name=:name  limit 1", array(
                ":uniacid" => $_W["uniacid"],
                ":module" => "manor_shop",
                ":name" => "manor_shop:article:" . $arr["id"]
            ));
            if (empty($rule)) {
                $rule_data = array(
                    "uniacid" => $_W["uniacid"],
        ...
        $kw    = $_GPC["kw"];
        $goods = pdo_fetchall("SELECT id,title,productprice,marketprice,thumb,hasoption FROM " . tablename("manor_shop_goods") . " WHERE uniacid= :uniacid and status=1 and deleted=0 AND title LIKE :title ", array(
            ":title" => "%{$kw}%",
            ":uniacid" => $_W["uniacid"]
        ));
        ...
        $articles = pdo_fetchall("SELECT a.id,a.article_title,a.article_category,c.category_name FROM " . tablename("manor_shop_article") . " a left join " . tablename("manor_shop_article_category") . " c on c.id=a.article_category WHERE a.uniacid= :uniacid " . $where . " and a.article_title LIKE :title ", array(
            ":title" => "%{$keyword}%",
            ":uniacid" => $_W["uniacid"]
        ));
        ...
        if (!empty($arr)) {
            ...
            $sys = pdo_fetch("select * from " . tablename("manor_shop_article_sys") . " where uniacid=:uniacid limit 1", array(
                ":uniacid" => $_W["uniacid"]
            ));
            if (empty($sys)) {
                $arr["uniacid"] = $_W["uniacid"];
                pdo_insert("manor_shop_article_sys", $arr);
            } else {
                pdo_update("manor_shop_article_sys", $arr, array(
                    "uniacid" => $_W["uniacid"]
                ));
            }
            die(json_encode(array(
