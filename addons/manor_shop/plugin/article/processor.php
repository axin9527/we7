...
class ArticleProcessor extends PluginProcessor
{
    ...
    public function respond($obj = null)
    {
        ...
        if ($msgtype == 'text' || $event == 'click') {
            $page = pdo_fetch('select * from ' . tablename('manor_shop_article') . ' where article_keyword=:keyword and article_state=1 and uniacid=:uniacid limit 1', array(
                ':uniacid' => $_W['uniacid'],
                ':keyword' => $content
            ));
            if (empty($page)) {
            ...
            $news    = array(
                array(
                    ...
                    'picurl' => $r_img,
                    'description' => $r_desc,
                    'url' => $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=manor_shop&do=plugin&p=article&aid=' . $page['id']
                )
            );
