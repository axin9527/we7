...
if ($operation == 'display'){
    ...
    $sql = 'select dm.*,dm.nickname,dm.avatar,l.levelname,p.nickname as parentname,p.avatar as parentavatar from ' . tablename('manor_shop_member') . ' dm ' . ' left join ' . tablename('manor_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('manor_shop_commission_level') . ' l on l.id = dm.agentlevel' . ' left join ' . tablename('mc_mapping_fans') . "f on f.openid=dm.openid and f.uniacid={$_W['uniacid']}" . ' where dm.uniacid = ' . $_W['uniacid'] . " and dm.isagent =1  {$condition} ORDER BY dm.agenttime desc";
    if(empty($_GPC['export'])){
        $sql .= ' limit ' . ($pindex - 1) * $psize . ',' . $psize;
    }
    $list = pdo_fetchall($sql , $params);
    $total = pdo_fetchcolumn('select count(dm.id) from' . tablename('manor_shop_member') . ' dm  ' . ' left join ' . tablename('manor_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid' . ' where dm.uniacid =' . $_W['uniacid'] . " and dm.isagent =1 {$condition}", $params);
    foreach ($list as & $row){
    ...
    pdo_update('manor_shop_member', array('isagent' => 0, 'status' => 0), array('id' => $_GPC['id']));
    ...
    if(!empty($black)){
        pdo_update('manor_shop_member', array('isagent' => 1, 'status' => 0, 'agentblack' => 1), array('id' => $_GPC['id']));
        ...
        pdo_update('manor_shop_member', array('isagent' => 1, 'status' => 1, 'agentblack' => 0), array('id' => $_GPC['id']));
    ...
    $level11 = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_member') . ' where isagent=0 and agentid=:agentid and uniacid=:uniacid limit 1', array(':agentid' => $agentid, ':uniacid' => $_W['uniacid']));
    $condition = '';
    $params = array();
    if (empty($level)){
    ...
    $list = array();
    if ($hasagent){
        $total = pdo_fetchcolumn('select count(dm.id) from' . tablename('manor_shop_member') . ' dm ' . ' left join ' . tablename('manor_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid' . ' where dm.uniacid =' . $_W['uniacid'] . "  {$condition}", $params);
        $list = pdo_fetchall('select dm.*,p.nickname as parentname,p.avatar as parentavatar  from ' . tablename('manor_shop_member') . ' dm ' . ' left join ' . tablename('manor_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('mc_mapping_fans') . "f on f.openid=dm.openid  and f.uniacid={$_W['uniacid']}" . ' where dm.uniacid = ' . $_W['uniacid'] . "  {$condition}   ORDER BY dm.agenttime desc limit " . ($pindex - 1) * $psize . ',' . $psize, $params);
        $pager = pagination($total, $pindex, $psize);
        foreach ($list as & $row){
            $info = $this -> model -> getInfo($row['openid'], array('total', 'pay'));
            $row['levelcount'] = $info['agentcount'];
    ...
    $ds = pdo_fetchall('SELECT id,avatar,nickname,openid,realname,mobile FROM ' . tablename('manor_shop_member') . " WHERE 1 {$condition} order by createtime desc", $params);
    ...
    $time = time();
    pdo_update('manor_shop_member', array('status' => 1, 'agenttime' => $time), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
    $this -> model -> sendMessage($member['openid'], array('nickname' => $member['nickname'], 'agenttime' => $time), TM_COMMISSION_BECOME);
    if(!empty($member['agentid'])){
        $this -> model -> upgradeLevelByAgent($member['agentid']);
    }
