...
if ($operation == 'display'){
    ...
    $sql = 'select a.*, m.nickname,m.avatar,m.realname,m.mobile,l.levelname from ' . tablename('manor_shop_commission_apply') . ' a ' . ' left join ' . tablename('manor_shop_member') . ' m on m.id = a.mid' . ' left join ' . tablename('manor_shop_commission_level') . ' l on l.id = m.agentlevel' . " where 1 {$condition} ORDER BY {$orderby} desc ";
    ...
    if ($_GPC['export'] == '1'){
        ...
        foreach ($list as & $row){
            ...
            $orders = pdo_fetchall('select id,agentid, ordersn,price,goodsprice,dispatchprice,createtime, paytype from ' . tablename('manor_shop_order') . ' where  id in ( ' . implode(',', $ids) . ' );');
            $totalcommission = 0;
            $totalpay = 0;
            $ordergoodscount = 0;
    ...
    $total = pdo_fetchcolumn('select count(a.id) from' . tablename('manor_shop_commission_apply') . ' a ' . ' left join ' . tablename('manor_shop_member') . ' m on m.uid = a.mid' . ' left join ' . tablename('manor_shop_commission_level') . ' l on l.id = m.agentlevel' . " where 1 {$condition}", $params);
    $pager = pagination($total, $pindex, $psize);
}else if ($operation == 'detail'){
    $id = intval($_GPC['id']);
    $apply = pdo_fetch('select * from ' . tablename('manor_shop_commission_apply') . ' where uniacid=:uniacid and id=:id limit 1' , array(':uniacid' => $_W['uniacid'], ':id' => $id));
    if (empty($apply)){
    ...
    $list = pdo_fetchall('select id,agentid, ordersn,price,goodsprice, dispatchprice,createtime, paytype from ' . tablename('manor_shop_order') . ' where  id in ( ' . implode(',', $ids) . ' );');
    $totalcommission = 0;
    $totalpay = 0;
    foreach ($list as & $row){
        foreach ($orderids as $o){
    ...
    unset($row);
    $totalcount = $total = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_order') . ' o ' . ' left join ' . tablename('manor_shop_member') . ' m on o.openid = m.openid ' . ' left join ' . tablename('manor_shop_member_address') . ' a on a.id = o.addressid ' . ' where o.id in ( ' . implode(',', $ids) . ' );');
    if (checksubmit('submit_check') && $apply['status'] == 1){
        ca('commission.apply.check');
        $paycommission = 0;
        $ogids = array();
        foreach ($list as $row){
            $goods = pdo_fetchall('SELECT id from ' . tablename('manor_shop_order_goods') . ' where uniacid = :uniacid and orderid=:orderid and nocommission=0', array(':uniacid' => $_W['uniacid'], ':orderid' => $row['id']));
            foreach ($goods as $g){
                $ogids[] = $g['id'];
            }
        }
        ...
        foreach ($ogids as $ogid){
            $g = pdo_fetch('SELECT total, commission1,commission2,commission3,commissions from ' . tablename('manor_shop_order_goods') . '  ' . 'where id=:id and uniacid = :uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $ogid));
            if (empty($g)){
                continue;
            }
            $commissions = iunserializer($g['commissions']);
            ...
            if (!empty($update)){
                pdo_update('manor_shop_order_goods', $update, array('id' => $ogid));
            }
        }
        if ($isAllUncheck){
            pdo_update('manor_shop_commission_apply', array('status' => -1, 'invalidtime' => $time), array('id' => $id, 'uniacid' => $_W['uniacid']));
        }else{
            pdo_update('manor_shop_commission_apply', array('status' => 2, 'checktime' => $time), array('id' => $id, 'uniacid' => $_W['uniacid']));
...
if (checksubmit('submit_cancel') && ($apply['status'] == 2 || $apply['status'] == -1)){
    ...
    foreach ($list as $row){
        ...
        foreach ($row['goods'] as $g){
            ...
            if (!empty($update)){
                pdo_update('manor_shop_order_goods', $update, array('id' => $g['id']));
            }
        }
    }
    pdo_update('manor_shop_commission_apply', array('status' => 1, 'checktime' => 0, 'invalidtime' => 0), array('id' => $id, 'uniacid' => $_W['uniacid']));
...
if (checksubmit('submit_pay') && $apply['status'] == 2){
    ...
    pdo_update('manor_shop_commission_apply', $updateno, array('id' => $apply['id']));
    ...
    pdo_update('manor_shop_commission_apply', array('status' => 3, 'paytime' => $time, 'commission_pay' => $totalpay), array('id' => $id, 'uniacid' => $_W['uniacid']));
    ...
    pdo_insert('manor_shop_commission_log', $log);
    ...
    $id = intval($_GPC['id']);
    $set = $this -> getSet();
    $order = pdo_fetch('select * from ' . tablename('manor_shop_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
    if (empty($order)){
        exit('fail');
    }
    ...
    foreach ($order_goods_change as $og){
        ...
        pdo_update('manor_shop_order_goods', array('commissions' => iserializer($commissions)), array('id' => $og['id']));
    }
