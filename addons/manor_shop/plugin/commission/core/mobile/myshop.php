...
if ($op == 'display'){
    if ($_W['isajax']){
        if(empty($shop['selectgoods'])){
            $goodscount = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_goods') . ' where uniacid=:uniacid and status=1 and deleted=0', array(':uniacid' => $_W['uniacid']));
        }else{
    ...
    if ($_W['isajax']){
        if ($_W['ispost']){
            ...
            if (empty($shop['id'])){
                pdo_insert('manor_shop_commission_shop', $shopdata);
            }else{
                pdo_update('manor_shop_commission_shop', $shopdata, array('id' => $shop['id']));
            }
            show_json(1);
        }
    ...
    if ($_W['isajax']){
        ...
        if(!empty($shop['selectgoods'])){
            $goodsids = explode(',', $shop['goodsids']);
            if(!empty($goodsids)){
                $goods = pdo_fetchall('select id,title,marketprice,thumb from ' . tablename('manor_shop_goods') . ' where uniacid=:uniacid and id in ( ' . trim($shop['goodsids']) . ')', array(':uniacid' => $_W['uniacid']));
