...
if ($_W['isajax']){
    ...
    if (empty($set['become'])){
        ...
        if (empty($become_reg)){
            ...
            pdo_update('manor_shop_member', $data, array('id' => $member['id']));
            if ($become_check == 1){
        ...
        $ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_order') . " where uniacid=:uniacid and openid=:openid and status>={$status} limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
        ...
        $moneycount = pdo_fetchcolumn('select sum(goodsprice) from ' . tablename('manor_shop_order') . " where uniacid=:uniacid and openid=:openid and status>={$status} limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
        ...
        $goods = pdo_fetch('select id,title from' . tablename('manor_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $set['become_goodsid'], ':uniacid' => $_W['uniacid']));
        $goodscount = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_order_goods') . ' og ' . '  left join ' . tablename('manor_shop_order') . ' o on o.id = og.orderid' . ' where og.goodsid=:goodsid and o.openid=:openid and o.status>=1  limit 1', array(':goodsid' => $set['become_goodsid'], ':openid' => $openid));
        if($goodscount <= 0){
            $ret['status'] = 3;
    ...
    if ($_W['ispost']){
        ...
        if ($ret['status'] == 1 || $ret['status'] == 2){
            ...
            if ($template_flag == 1){
                ...
                pdo_update('manor_shop_member', $m_data, array('id' => $member['id']));
                if ($become_check == 1){
