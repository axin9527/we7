...
if ($_W['isajax']){
    ...
    $list = array();
    if ($hasangent){
        $list = pdo_fetchall("select * from " . tablename('manor_shop_member') . " where isagent =1 and status=1 and uniacid = " . $_W['uniacid'] . " {$condition}  ORDER BY agenttime desc limit " . ($pindex - 1) * $psize . ',' . $psize);
        foreach ($list as & $row){
