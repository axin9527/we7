...
if ($operation == 'display'){
    ...
    $list = pdo_fetchall("SELECT u.*,r.rolename FROM " . tablename('manor_shop_perm_user') . " u  " . " left join " . tablename('manor_shop_perm_role') . " r on u.roleid =r.id  " . " WHERE 1 {$condition} ORDER BY id desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
    $total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('manor_shop_perm_user') . " u  " . " left join " . tablename('manor_shop_perm_role') . " r on u.roleid =r.id  " . " WHERE 1 {$condition} ", $params);
    $pager = pagination($total, $pindex, $psize);
    $roles = pdo_fetchall('select id,rolename from ' . tablename('manor_shop_perm_role') . ' where uniacid=:uniacid and deleted=0', array(':uniacid' => $_W['uniacid']));
    ...
    $item = pdo_fetch("SELECT * FROM " . tablename('manor_shop_perm_user') . " WHERE id =:id and deleted=0 and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':id' => $id));
    ...
    if(!empty($item)){
        ...
        $role = pdo_fetch("SELECT * FROM " . tablename('manor_shop_perm_role') . " WHERE id =:id and deleted=0 and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':id' => $item['roleid']));
        if(!empty($role)){
    ...
    if ($_W['isajax'] && $_W['ispost']){
        ...
        if (!empty($item['id'])){
            ...
            pdo_update('manor_shop_perm_user', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
            ...
            pdo_insert('manor_shop_perm_user', $data);
            $id = pdo_insertid();
    ...
    ca('perm.user.delete');
    $id = intval($_GPC['id']);
    $item = pdo_fetch("SELECT id,uid,username FROM " . tablename('manor_shop_perm_user') . " WHERE id = '$id'");
    if (empty($item)){
        message('抱歉，操作员不存在或是已经被删除！', $this -> createPluginWebUrl('perm/user', array('op' => 'display')), 'error');
    }
    pdo_delete('manor_shop_perm_user', array('id' => $id, 'uniacid' => $_W['uniacid']));
