...
if (!class_exists('CreditshopModel')){
    class CreditshopModel extends PluginModel{
        public function getGoods($id, $member){
            ...
            $goods = pdo_fetch('select * from ' . tablename('manor_shop_creditshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
            if (empty($goods)){
                return array('canbuy' => false, 'buymsg' => '已下架');
            }
            ...
            if (empty($goods['canbuy'])){
                ...
                if ($goods['total'] > 0){
                    $logcount = pdo_fetchcolumn('select count(*)  from ' . tablename('manor_shop_creditshop_log') . "  where goodsid=:goodsid and status>=2  and uniacid=:uniacid  ", array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
                ...
                if ($goods['canbuy']){
                    if ($goods['totalday'] > 0){
                        $logcount = pdo_fetchcolumn('select count(*)  from ' . tablename('manor_shop_creditshop_log') . "  where goodsid=:goodsid and status>=2 and  date_format(from_UNIXTIME(`createtime`),'%Y-%m-%d') = date_format(now(),'%Y-%m-%d') and uniacid=:uniacid  ", array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
                ...
                if ($goods['canbuy']){
                    if ($goods['chanceday'] > 0){
                        $logcount = pdo_fetchcolumn('select count(*)  from ' . tablename('manor_shop_creditshop_log') . "  where goodsid=:goodsid and openid=:openid and status>0 and  date_format(from_UNIXTIME(`createtime`),'%Y-%m-%d') = date_format(now(),'%Y-%m-%d') and uniacid=:uniacid  ", array(':goodsid' => $id, ':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
                ...
                if ($goods['canbuy']){
                    if ($goods['chance'] > 0){
                        $logcount = pdo_fetchcolumn('select count(*)  from ' . tablename('manor_shop_creditshop_log') . '  where goodsid=:goodsid and openid=:openid and status>0 and  uniacid=:uniacid  ', array(':goodsid' => $id, ':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
                ...
                if ($goods['canbuy']){
                    if ($goods['usermaxbuy'] > 0){
                        $logcount = pdo_fetchcolumn('select ifnull(sum(total),0)  from ' . tablename('manor_shop_creditshop_log') . '  where goodsid=:goodsid and openid=:openid  and uniacid=:uniacid ', array(':goodsid' => $id, ':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
                        if ($logcount >= $goods['chance']){
                            $goods['canbuy'] = false;
                            $goods['buymsg'] = '已参加';
                        }
        ...
        public function createENO(){
            global $_W;
            $ecount = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_creditshop_log') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
            if($ecount < 99999999){
                $ecount = 8;
            }else{
            ...
            while(1){
                $c = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_creditshop_log') . ' where uniacid=:uniacid and eno=:eno limit 1', array(':uniacid' => $_W['uniacid'], ':eno' => $eno));
                if($c <= 0){
                    break;
                }
                $eno = rand(pow(10, $ecount), pow(10, $ecount + 1)-1);
        ...
        public function sendMessage($id = 0){
            ...
            $log = pdo_fetch('select * from ' . tablename('manor_shop_creditshop_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
            if (empty($log)){
                return;
            }
            ...
            $set = $this -> getSet();
            $tm = $set['tm'];
            $detailurl = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=manor_shop&do=plugin&p=creditshop&method=log&op=detail&&id=' . $id;
            if (strexists($detailurl, '/addons/manor_shop/')){
                $detailurl = str_replace("/addons/manor_shop/", '/', $detailurl);
            }
            if($log['status'] == 2){
                if(!empty($type)){
                ...
                if($log['dispatchstatus'] == 1 || $log['dispatchstatus'] == -1){
                    ...
                    if(!empty($log['addressid'])){
                        $address = pdo_fetch('select id,realname,mobile,address,province,city,area from ' . tablename('manor_shop_member_address') . ' where id=:id and uniacid=:uniacid limit 1' , array(':id' => $log['addressid'], ':uniacid' => $_W['uniacid']));
                        if(!empty($address)){
                ...
                $detailurl1 = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&method=detail&p=creditshop&m=manor_shop&do=plugin&id=' . $log['goodsid'];
                if (strexists($detailurl1, '/addons/manor_shop/')){
                    $detailurl1 = str_replace("/addons/manor_shop/", '/', $detailurl1);
                }
        ...
        public function createQrcode($logid = 0){
            global $_W, $_GPC;
            $path = IA_ROOT . "/addons/manor_shop/data/creditshop/" . $_W['uniacid'];
            if (!is_dir($path)){
                load() -> func('file');
                mkdirs($path);
            }
            $url = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=manor_shop&do=plugin&p=creditshop&method=exchange&op=exchange&id=' . $logid;
            ...
            return $_W['siteroot'] . '/addons/manor_shop/data/creditshop/' . $_W['uniacid'] . '/' . $file;
        }
        function perms(){
