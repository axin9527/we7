...
if ($operation == 'display'){
    ...
    $sql = 'select log.*, m.nickname,m.avatar,m.realname as mrealname,m.mobile as mmobile, g.title,g.thumb,g.thumb,g.credit,g.money,g.type as goodstype,g.isverify,g.goodstype as iscoupon,s.storename,s.address as storeaddress from ' . tablename('manor_shop_creditshop_log') . ' log ' . ' left join ' . tablename('manor_shop_member') . ' m on m.openid = log.openid and m.uniacid=log.uniacid' . ' left join ' . tablename('manor_shop_member_address') . ' a on a.id = log.addressid' . ' left join ' . tablename('manor_shop_store') . ' s on s.id = log.storeid' . ' left join ' . tablename('manor_shop_creditshop_goods') . ' g on g.id = log.goodsid' . " where 1 {$condition} ORDER BY log.createtime desc ";
    if (empty($_GPC['export'])){
        $sql .= ' limit ' . ($pindex - 1) * $psize . ',' . $psize;
    }
    $list = pdo_fetchall($sql, $params);
    $total = pdo_fetchcolumn('select count(log.id) from' . tablename('manor_shop_creditshop_log') . ' log ' . ' left join ' . tablename('manor_shop_member') . ' m on m.openid = log.openid and m.uniacid=log.uniacid' . ' left join ' . tablename('manor_shop_store') . ' s on s.id = log.storeid' . ' left join ' . tablename('manor_shop_member_address') . ' a on a.id = log.addressid' . ' left join ' . tablename('manor_shop_creditshop_goods') . ' g on g.id = log.goodsid' . " where 1 {$condition}", $params);
    foreach ($list as & $row){
        $row['address'] = array();
        if (!empty($row['addressid'])){
            $row['address'] = pdo_fetch('select realname,mobile,address,province,city,area from ' . tablename('manor_shop_member_address') . ' where id=:id and uniacid=:uniacid limit 1' , array(':id' => $row['addressid'], ':uniacid' => $_W['uniacid']));
        }else{
    ...
    pdo_update('manor_shop_creditshop_log', array('status' => 3, 'usetime' => time(), 'expresscom' => $_GPC['expresscom'], 'expresssn' => $_GPC['expresssn'], 'express' => $_GPC['express']), array('id' => $id));
