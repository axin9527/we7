...
ca('creditshop.cover');
$rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => "manor_shop积分商城入口设置"));
if (!empty($rule)){
...
if (checksubmit('submit')){
    ...
    $rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'manor_shop积分商城入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($data['status']));
