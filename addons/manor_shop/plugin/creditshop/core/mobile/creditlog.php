...
if ($_W['isajax']){
    if ($operation == 'display'){
        ...
        $list = array();
        if (!empty($total)){
            $sql = 'SELECT log.id,log.goodsid,g.title,g.thumb,g.credit,g.type,g.money,log.createtime FROM ' . tablename('manor_shop_creditshop_log') . ' log ' . ' left join ' . tablename('manor_shop_creditshop_goods') . ' g on log.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
