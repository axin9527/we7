...
if ($operation == 'display') {
    ...
    if (!empty($_GPC['catname'])) {
        ...
        foreach ($_GPC['catname'] as $id => $catname) {
            if ($id == 'new') {
                ...
                pdo_insert('manor_shop_diyform_category', array(
                    'name' => $catname,
                    'uniacid' => $_W['uniacid']
                ));
                ...
                pdo_update('manor_shop_diyform_category', array(
                    'name' => $catname
                ), array(
                    'id' => $id
                ));
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename('manor_shop_diyform_category') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY id DESC");
} elseif ($operation == 'delete') {
    ca('diyform.category.delete');
    $id   = intval($_GPC['id']);
    $item = pdo_fetch("SELECT id,name FROM " . tablename('manor_shop_diyform_category') . " WHERE id = '$id' AND uniacid=" . $_W['uniacid'] . "");
    if (empty($item)) {
        message('抱歉，分类不存在或是已经被删除！', $this->createPluginWebUrl('diyform/category', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('manor_shop_diyform_category', array(
        'id' => $id
    ));
