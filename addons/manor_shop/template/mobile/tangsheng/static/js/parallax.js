...
! function($) {
  ...
  function init() {
    ...
    console.log(options,'23456789..............');
    curPage = options.indexpage;
    console.log(curPage,'23456...........');
    direction = 'stay';
    ...
    if (options.swipeAnim === 'cover') { // 重置 page 的宽高(因为这两个效果与 default 实现方式截然不同)
      ...
      $pageArr[curPage].style.display = 'block'; // 不能通过 css 来定义，不然在 Android 和 iOS 下会有 bug
    }
  ...
  function onEnd(e) {
    ...
    if (movePrevent === true || stage !== 2) {
      ...
      if (options.indicator) {
        $($('.parallax-h-indicator ul li, .parallax-v-indicator ul li').removeClass(
          'current').get(curPage)).addClass('current');
      }
      stage = 3;
    }
  ...
  function animShow() {
    ...
    $('.current [data-animation]').each(function(index, element) {
      var $element = $(element),
        ...
        $delay = $element.attr('data-delay') ? $element.attr('data-delay') :
        0;
      ...
      if ($animation === 'followSlide') {
        ...
        } else if (options.direction === 'horizontal' && direction ===
          'backward') {
          $animation = 'followSlideToRight';
        } else if (options.direction === 'vertical' && direction ===
          'forward') {
  ...
  $(window).on("load", function() {
    ...
    if (options&&options.indicator) {
      ...
      var temp = options.direction === 'horizontal' ?
        'parallax-h-indicator' : 'parallax-v-indicator';
