...
class Core extends WeModuleSite{
    ...
    public function runTasks(){
        ...
        if ($lasttime + $interval <= $current){
            ...
            ihttp_request($_W['siteroot'] . "addons/manor_shop/core/task/order/receive.php", null, null, 1);
        }
        ...
        if ($lasttime + $interval <= $current){
            ...
            ihttp_request($_W['siteroot'] . "addons/manor_shop/core/task/order/close.php", null, null, 1);
        }
        if(p('coupon')){
            ...
            if ($lasttime + $interval <= $current){
                ...
                ihttp_request($_W['siteroot'] . "addons/manor_shop/core/task/coupon/back.php", null, null, 1);
            }
            //优惠劵提醒通知
            $noticelasttime = strtotime(m('cache') -> getString('couponnoticetime', 'global'));
            $notice_interval = intval($pset['expiredruntime']);
            if(!$notice_interval) {
                $notice_interval = 60;
            }
            $notice_interval *= 60;
            if ($noticelasttime + $notice_interval <= $current){
                m('cache') -> set('couponnoticetime', date('Y-m-d H:i:s', $current), 'global');
                ihttp_request($_W['siteroot'] . "addons/manor_shop/core/task/coupon/expired.php", null, null, 1);
            }
        }
    ...
    public function setHeader(){
        ...
        if (!$followed && $memberid != $mid){
            ...
            $friend = false;
            if (!empty($mid)){
                if (!empty($_SESSION[manor_shop_PREFIX . '_shareid']) && $_SESSION[manor_shop_PREFIX . '_shareid'] == $mid){
                    $mid = $_SESSION[manor_shop_PREFIX . '_shareid'];
                }
                $member = m('member') -> getMember($mid);
                if (!empty($member)){
                    $_SESSION[manor_shop_PREFIX . '_shareid'] = $mid;
                    $friend = true;
    ...
    public function setFooter(){
        global $_GPC,$_W;
        $openid = m('user') -> getOpenid();
        $cart_num = pdo_fetchcolumn('select sum(total) from ' . tablename('manor_shop_member_cart') . ' where uniacid=:uniacid and openid=:openid and deleted=0 ', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
        $this->footer['cart_num'] = $cart_num;
    ...
    public function _exec($do, $default = '', $web = true){
        ...
        empty($p) && $p = $default;
        if ($web){
            $file = IA_ROOT . "/addons/manor_shop/core/web/" . $do . "/" . $p . ".php";
        }else{
            $this -> setFooter();
            $file = IA_ROOT . "/addons/manor_shop/core/mobile/" . $do . "/" . $p . ".php";
        }
        if (!is_file($file)){
    ...
    public function template($filename, $type = TEMPLATE_INCLUDEPATH){
        ...
        if (defined('IN_SYS')){
            ...
            if (!is_dir(IA_ROOT . '/addons/manor_shop/template/mobile/' . $template)){
                $template = "default";
            }
            $compile = IA_ROOT . "/data/tpl/app/manor_shop/{$template}/mobile/{$filename}.tpl.php";
            ...
            if (!is_file($source)){
                ...
                if (!is_dir(IA_ROOT . '/addons/manor_shop/plugin/' . $pluginname . "/template/mobile/" . $ptemplate)){
                    $ptemplate = "default";
                }
                $pfilename = $names[1];
                $source = IA_ROOT . "/addons/manor_shop/plugin/" . $pluginname . "/template/mobile/" . $ptemplate . "/{$pfilename}.html";
            }
            if (!is_file($source)){
