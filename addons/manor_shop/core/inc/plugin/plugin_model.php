...
class PluginModel{
    ...
    public function updateSet($data = array()){
        ...
        if (empty($set)){
            pdo_insert('manor_shop_sysset', array('uniacid' => $uniacid, 'sets' => iserializer(array()), 'plugins' => iserializer(array($this -> pluginname => $data))));
        }else{
            $sets = unserialize($set['plugins']);
            $sets[$this -> pluginname] = $data;
            pdo_update('manor_shop_sysset', array('plugins' => iserializer($sets)), array('uniacid' => $uniacid));
        }
        $set = pdo_fetch("select * from " . tablename('manor_shop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
        m('cache') -> set('sysset', $set);
    }
    function getName(){
        return pdo_fetchcolumn('select name from ' . tablename('manor_shop_plugin') . ' where identity=:identity limit 1', array(':identity' => $this -> pluginname));
    }
}
