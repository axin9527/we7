...
class Ewei_DShop_Member{
    public function getInfo($openid = ''){
        ...
        $uid = intval($openid);
        if ($uid == 0){
            $info = pdo_fetch('select * from ' . tablename('manor_shop_member') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
        }else{
            $info = pdo_fetch('select * from ' . tablename('manor_shop_member') . ' where id=:id  and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $uid));
        }
        if (!empty($info['uid'])){
            load() -> model('mc');
            ...
            if (empty($info['uid'])){
                ...
                if ($followed){
                    ...
                    if (!empty($uid)){
                        ...
                        if (!empty($upgrade)){
                            pdo_update('manor_shop_member', $upgrade, array('id' => $info['id']));
                        }
                    }
                }
            }
    ...
    public function getCredits($openid = '', $credittypes = array('credit1', 'credit2')){
        ...
        if (!empty($uid)){
            ...
            return pdo_fetch("SELECT {$types} FROM " . tablename('manor_shop_member') . " WHERE  openid=:openid and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
        }
    }
    public function checkMember($openid = ''){
        ...
        if (empty($member)){
            ...
            pdo_insert('manor_shop_member', $member);
        }else{
            if($member['isblack'] == 1){
                die("<!DOCTYPE html>
						<html>
            ...
            if (!empty($upgrade)){
                pdo_update('manor_shop_member', $upgrade, array('id' => $member['id']));
            }
        }
        if (p('commission')){
    ...
    function getLevels(){
        global $_W;
        return pdo_fetchall('select * from ' . tablename('manor_shop_member_level') . ' where uniacid=:uniacid order by level asc', array(':uniacid' => $_W['uniacid']));
    }
    function getLevel($openid){
        ...
        $level = pdo_fetch('select * from ' . tablename('manor_shop_member_level') . ' where id=:id and uniacid=:uniacid order by level asc', array(':uniacid' => $_W['uniacid'], ':id' => $member['level']));
        if (empty($level)){
            return array('discount' => $shop['leveldiscount']);
        }
        return $level;
    ...
    function upgradeLevel($openid){
        ...
        $level = false;
        if (empty($leveltype)){
            $ordermoney = pdo_fetchcolumn('select ifnull( sum(og.realprice),0) from ' . tablename('manor_shop_order_goods') . ' og ' . ' left join ' . tablename('manor_shop_order') . ' o on o.id=og.orderid ' . ' where o.openid=:openid and o.status=3 and o.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
            $level = pdo_fetch('select * from ' . tablename('manor_shop_member_level') . " where uniacid=:uniacid  and {$ordermoney} >= ordermoney and ordermoney>0  order by level desc limit 1", array(':uniacid' => $_W['uniacid']));
        }else if ($leveltype == 1){
            $ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_order') . ' where openid=:openid and status=3 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
            $level = pdo_fetch('select * from ' . tablename('manor_shop_member_level') . " where uniacid=:uniacid  and {$ordercount} >= ordercount and ordercount>0  order by level desc limit 1", array(':uniacid' => $_W['uniacid']));
        }
        if (empty($level)){
            return;
        }
        ...
        if ($canupgrade){
            pdo_update('manor_shop_member', array('level' => $level['id']), array('id' => $member['id']));
            m('notice') -> sendMemberUpgradeMessage($openid, $oldlevel, $level);
        }
    }
    function getGroups(){
        global $_W;
        return pdo_fetchall('select * from ' . tablename('manor_shop_member_group') . ' where uniacid=:uniacid order by id asc', array(':uniacid' => $_W['uniacid']));
    }
