...
class Ewei_DShop_Common{
    ...
    public function alipay_build($params, $alipay = array(), $type = 0, $openid = ''){
        ...
        $set['sign_type'] = 'MD5';
        if (empty($type)){
            $set['notify_url'] = $_W['siteroot'] . "addons/manor_shop/payment/alipay/notify.php";
            $set['return_url'] = $_W['siteroot'] . "app/index.php?i={$_W['uniacid']}&c=entry&m=manor_shop&do=order&p=pay&op=return&openid=" . $openid;
        }else{
            $set['notify_url'] = $_W['siteroot'] . "addons/manor_shop/payment/alipay/notify.php";
            $set['return_url'] = $_W['siteroot'] . "app/index.php?i={$_W['uniacid']}&c=entry&m=manor_shop&do=member&p=recharge&op=return&openid=" . $openid;
        }
    ...
    public function unionpay_build($params, $unionpay = array(), $type = 0, $openid = '')
    {
        global $_W;
        $set = array(
            'version' => '5.0.0',
            ...
            'orderId' => $params['uniontid'],
            'txnTime' => date('YmdHis'),
            'txnAmt' => $params['fee'] * 100,
            'currencyCode' => '156',
            'defaultPayType' => '0001',
            'reqReserved' => $_W['uniacid'],
        );
        if (empty($type)){
            $set['frontUrl'] = $_W['siteroot'] . "addons/manor_shop/payment/unionpay/notify.php";
            $set['backUrl'] = $_W['siteroot'] . "app/index.php?i={$_W['uniacid']}&c=entry&m=manor_shop&do=order&p=pay&op=return&openid=" . $openid;
        }else{
            $set['frontUrl'] = $_W['siteroot'] . "addons/manor_shop/payment/unionpay/notify.php";
            $set['backUrl'] = $_W['siteroot'] . "app/index.php?i={$_W['uniacid']}&c=entry&m=manor_shop&do=member&p=recharge&op=return&openid=" . $openid;
        }
        return $set;
    }
    ...
    function wechat_build($params, $wechat, $type = 0){
        ...
        if ($wechat['version'] == 1){
            ...
            $package['device_info'] = "manor_shop";
            $package['out_trade_no'] = $params['tid'];
            $package['total_fee'] = $params['fee'] * 100;
            $package['fee_type'] = '1';
            $package['notify_url'] = $_W['siteroot'] . "addons/manor_shop/payment/wechat/notify.php";
            ...
            $response = ihttp_post('https://api.mch.weixin.qq.com/pay/unifiedorder', $dat);
    ...
    public function getSec($uniacid = 0){
        ...
        $set = pdo_fetch("select sec from " . tablename('manor_shop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
        if (empty($set)){
            $set = array();
        }
        return $set;
    ...
    public function paylog($log = ''){
        ...
        if (!empty($openpaylog)){
            $path = IA_ROOT . "/addons/manor_shop/data/paylog/" . $_W['uniacid'] . "/" . date('Ymd');
            if (!is_dir($path)){
                load() -> func('file');
                @mkdirs($path, '0777');
            }
