...
function field_index($dephp_0, $dephp_1){
    $dephp_2 = -1;
    foreach ($dephp_0 as $dephp_3 => $dephp_4){
        if ($dephp_4['field'] == $dephp_1){
            $dephp_2 = $dephp_3;
            break;
        }
    }
...
if ($_GPC['export'] == 1){
    ...
    $sql = 'select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea,a.address as aaddress, d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,sm.id as salerid,sm.nickname as salernickname,s.salername from ' . tablename('manor_shop_order') . ' o' . ' left join ( select rr.id,rr.orderid,rr.status from ' . tablename('manor_shop_order_refund') . ' rr left join ' . tablename('manor_shop_order') . ' ro on rr.orderid =ro.id order by rr.id desc limit 1) r on r.orderid= o.id' . ' left join ' . tablename('manor_shop_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid ' . ' left join ' . tablename('manor_shop_member_address') . ' a on a.id=o.addressid ' . ' left join ' . tablename('manor_shop_dispatch') . ' d on d.id = o.dispatchid ' . ' left join ' . tablename('manor_shop_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' left join ' . tablename('manor_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . " where $condition $statuscondition ORDER BY o.createtime DESC,o.status DESC  ";
...
';
        ...
        if (!empty($value['verifystoreid'])){
            $value['storeinfo'] = pdo_fetchcolumn('select storename from ' . tablename('manor_shop_store') . ' where id=:storeid limit 1 ', array(':storeid' => $value['verifystoreid']));
        }
...
$condition = '1';
$totals = array();
$totals['all'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('manor_shop_order') . ' o ' . ' left join ( select rr.id,rr.orderid,rr.status from ' . tablename('manor_shop_order_refund') . ' rr left join ' . tablename('manor_shop_order') . ' ro on rr.orderid =ro.id order by rr.id desc limit 1) r on r.orderid= o.id' . ' left join ' . tablename('manor_shop_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('manor_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('manor_shop_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' left join ' . tablename('manor_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' WHERE o.uniacid = :uniacid and o.deleted=0', $paras);
$totals['status_1'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('manor_shop_order') . ' o ' . ' left join ( select rr.id,rr.orderid,rr.status from ' . tablename('manor_shop_order_refund') . ' rr left join ' . tablename('manor_shop_order') . ' ro on rr.orderid =ro.id order by rr.id desc limit 1) r on r.orderid= o.id' . ' left join ' . tablename('manor_shop_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('manor_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('manor_shop_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' left join ' . tablename('manor_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . " WHERE $condition and o.status=-1 and o.refundtime=0", $paras);
$totals['status0'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('manor_shop_order') . ' o ' . ' left join ( select rr.id,rr.orderid,rr.status from ' . tablename('manor_shop_order_refund') . ' rr left join ' . tablename('manor_shop_order') . ' ro on rr.orderid =ro.id order by rr.id desc limit 1) r on r.orderid= o.id' . ' left join ' . tablename('manor_shop_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('manor_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('manor_shop_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' left join ' . tablename('manor_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . " WHERE $condition and o.status=0 and o.paytype<>3", $paras);
$totals['status1'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('manor_shop_order') . ' o ' . ' left join ( select rr.id,rr.orderid,rr.status from ' . tablename('manor_shop_order_refund') . ' rr left join ' . tablename('manor_shop_order') . ' ro on rr.orderid =ro.id order by rr.id desc limit 1) r on r.orderid= o.id' . ' left join ' . tablename('manor_shop_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('manor_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('manor_shop_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' left join ' . tablename('manor_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . " WHERE $condition and ( o.status=1 or ( o.status=0 and o.paytype=3) )", $paras);
$totals['status2'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('manor_shop_order') . ' o ' . ' left join ( select rr.id,rr.orderid,rr.status from ' . tablename('manor_shop_order_refund') . ' rr left join ' . tablename('manor_shop_order') . ' ro on rr.orderid =ro.id order by rr.id desc limit 1) r on r.orderid= o.id' . ' left join ' . tablename('manor_shop_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('manor_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('manor_shop_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' left join ' . tablename('manor_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . " WHERE $condition and o.status=2", $paras);
$totals['status3'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('manor_shop_order') . ' o ' . ' left join ( select rr.id,rr.orderid,rr.status from ' . tablename('manor_shop_order_refund') . ' rr left join ' . tablename('manor_shop_order') . ' ro on rr.orderid =ro.id order by rr.id desc limit 1) r on r.orderid= o.id' . ' left join ' . tablename('manor_shop_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('manor_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('manor_shop_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' left join ' . tablename('manor_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . " WHERE $condition and o.status=3", $paras);
$totals['status4'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('manor_shop_order') . ' o ' . ' left join ( select rr.id,rr.orderid,rr.status from ' . tablename('manor_shop_order_refund') . ' rr left join ' . tablename('manor_shop_order') . ' ro on rr.orderid =ro.id order by rr.id desc limit 1) r on r.orderid= o.id' . ' left join ' . tablename('manor_shop_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('manor_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('manor_shop_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' left join ' . tablename('manor_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . " WHERE $condition and o.refundid<>0", $paras);
$totals['status5'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('manor_shop_order') . ' o ' . ' left join ( select rr.id,rr.orderid,rr.status from ' . tablename('manor_shop_order_refund') . ' rr left join ' . tablename('manor_shop_order') . ' ro on rr.orderid =ro.id  order by rr.id desc limit 1) r on r.orderid= o.id' . ' left join ' . tablename('manor_shop_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('manor_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('manor_shop_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' left join ' . tablename('manor_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . " WHERE $condition and o.refundtime<>0", $paras);
load() -> func('tpl');
