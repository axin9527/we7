...
$sql = "SELECT m.realname, m.mobile,m.avatar,m.nickname,l.levelname,m.openid," . "(select ifnull( count(o.id) ,0) from  " . tablename('manor_shop_order') . " o where o.openid=m.openid and o.status>=1 {$condition})  as ordercount," . "(select ifnull(sum(o.price),0) from  " . tablename('manor_shop_order') . " o where o.openid=m.openid  and o.status>=1 {$condition})  as ordermoney" . " from " . tablename('manor_shop_member') . " m  " . " left join " . tablename('manor_shop_member_level') . " l on l.id = m.level" . " where 1 {$condition1} order by {$orderby} desc ";
if(empty($_GPC['export'])){
    $sql .= "LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
}
$list = pdo_fetchall($sql, $params1);
$total = pdo_fetchcolumn("select  count(*) from " . tablename('manor_shop_member') . ' m ' . " where 1 {$condition1} ", $params1);
...
if($list) {
    foreach ($list as $key=>$item) {
        $list[$key]['fanid'] = pdo_fetchcolumn('select fanid from '.tablename('mc_mapping_fans').' where openid=:openid and uniacid=:uniacid', array(':openid'=>$item['openid'], ':uniacid'=>$_W['uniacid']));
    }
}
load() -> func('tpl');
