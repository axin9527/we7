...
if (!empty($year) && !empty($month) && !empty($day)){
    ...
    for ($d = 1;$d <= $lastday;$d++){
        $dr = array('data' => $d, 'count' => pdo_fetchcolumn("SELECT ifnull({$countfield},0) as cnt FROM " . tablename('manor_shop_order') . " WHERE uniacid=:uniacid and status>=1 and createtime >=:starttime and createtime <=:endtime", array(':uniacid' => $_W['uniacid'], ':starttime' => strtotime("{$year}-{$month}-{$d} 00:00:00"), ':endtime' => strtotime("{$year}-{$month}-{$d} 23:59:59"))));
        $totalcount += $dr['count'];
        if ($dr['count'] > $maxcount){
            $maxcount = $dr['count'];
