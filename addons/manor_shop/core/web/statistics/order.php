...
$sql = 'select o.*, a.realname as addressname,m.realname from ' . tablename('manor_shop_order') . ' o ' . ' left join ' . tablename('manor_shop_member') . ' m on o.openid = m.openid ' . ' left join ' . tablename('manor_shop_member_address') . ' a on a.id = o.addressid ' . " where 1 {$condition} ";
...
foreach ($list as & $row){
    ...
    $row['goods'] = pdo_fetchall('SELECT g.thumb,og.price,og.total,og.realprice,g.title,og.optionname from ' . tablename('manor_shop_order_goods') . ' og' . ' left join ' . tablename('manor_shop_goods') . ' g on g.id=og.goodsid  ' . ' where og.uniacid = :uniacid and og.orderid=:orderid order by og.createtime  desc ', array(':uniacid' => $_W['uniacid'], ':orderid' => $row['id']));
    $totalmoney += $row['price'];
}
unset($row);
$totalcount = $total = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_order') . ' o ' . ' left join ' . tablename('manor_shop_member') . ' m on o.openid = m.openid ' . ' left join ' . tablename('manor_shop_member_address') . ' a on a.id = o.addressid ' . " where 1 {$condition}", $params);
