...
$sql = 'SELECT g.id,g.title,g.thumb,g.viewcount,' . '(select sum(og.total) from  ' . tablename('manor_shop_order_goods') . ' og left join ' . tablename('manor_shop_order') . " o on og.orderid=o.id  where o.status>=1 and og.goodsid=g.id {$condition})  as buycount" . ' from ' . tablename('manor_shop_goods') . ' g  ' . "where 1 {$condition1} order by buycount/g.viewcount {$orderby}  ";
