...
if ($operation == 'display'){
    ...
    if (!empty($_GPC['displayorder'])){
        ...
        foreach ($_GPC['displayorder'] as $id => $displayorder){
            pdo_update('manor_shop_dispatch', array('displayorder' => $displayorder), array('id' => $id));
        }
        plog('shop.dispatch.edit', '批量修改配送方式排序');
        message('分类排序更新成功！', $this -> createWebUrl('shop/dispatch', array('op' => 'display')), 'success');
    }
    $list = pdo_fetchall('SELECT * FROM ' . tablename('manor_shop_dispatch') . " WHERE uniacid = '{$_W['uniacid']}' and dispatchtype=0 ORDER BY id asc");
    ...
    $id = intval($_GPC['id']);
    $dispatch = pdo_fetch('SELECT id,dispatchname FROM ' . tablename('manor_shop_dispatch') . " WHERE id = '$id' AND uniacid=" . $_W['uniacid'] . "");
    if (empty($dispatch)){
        message('抱歉，配送方式不存在或是已经被删除！', $this -> createWebUrl('shop/dispatch', array('op' => 'display')), 'error');
    }
    pdo_delete('manor_shop_dispatch', array('id' => $id));
