...
if ($operation == 'display') {
    $list = pdo_fetchall('SELECT * FROM ' . tablename('manor_shop_express') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY displayorder DESC");
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            unset($data['parentid']);
            pdo_update('manor_shop_express', $data, array('id' => $id));
        } else {
            pdo_insert('manor_shop_express', $data);
            $id = pdo_insertid();
        }
        message('更新物流成功！', $this->createWebUrl('shop/express', array('op' => 'display')), 'success');
    }
    $express = pdo_fetch('SELECT * FROM ' . tablename('manor_shop_express') . " WHERE id = '{$id}' and uniacid = '{$_W['uniacid']}'");
} elseif ($operation == 'delete') {
    $id = intval($_GPC['id']);
    $express = pdo_fetch('SELECT id  FROM ' . tablename('manor_shop_express') . " WHERE id = '{$id}' AND uniacid=" . $_W['uniacid'] . '');
    if (empty($express)) {
        message('抱歉，物流方式不存在或是已经被删除！', $this->createWebUrl('shop/express', array('op' => 'display')), 'error');
    }
    pdo_delete('manor_shop_express', array('id' => $id));
