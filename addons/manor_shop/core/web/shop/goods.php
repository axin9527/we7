...
if ($operation == 'change') {
    ...
    $goods = pdo_fetch('select id from ' . tablename('manor_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(
        ':uniacid' => $_W['uniacid'],
        ':id' => $id
    ));
    if (empty($goods)) {
    ...
    pdo_update('manor_shop_goods', array(
        $type => $value
    ), array(
        'id' => $id
    ));
    ...
    if ($operation == 'post') {
        ...
        if (! empty($id)) {
            $item = pdo_fetch('SELECT * FROM ' . tablename('manor_shop_goods') . ' WHERE id = :id', array(
                ':id' => $id
            ));
            if (empty($item)) {
            ...
            $allspecs = pdo_fetchall('select * from ' . tablename('manor_shop_goods_spec') . ' where goodsid=:id order by displayorder asc', array(
                ':id' => $id
            ));
            foreach ($allspecs as & $s) {
                $s['items'] = pdo_fetchall('select a.id,a.specid,a.title,a.thumb,a.show,a.displayorder,a.valueId,a.virtual,b.title as title2 from ' . tablename('manor_shop_goods_spec_item') . ' a left join ' . tablename('manor_shop_virtual_type') . ' b on b.id=a.virtual  where a.specid=:specid order by a.displayorder asc', array(
                    ':specid' => $s['id']
                ));
            }
            unset($s);
            $params = pdo_fetchall('select * from ' . tablename('manor_shop_goods_param') . ' where goodsid=:id order by displayorder asc', array(
                ':id' => $id
            ));
            ...
            $html = "";
            $options = pdo_fetchall('select * from ' . tablename('manor_shop_goods_option') . ' where goodsid=:id order by id asc', array(
                ':id' => $id
            ));
            $specs = array();
            if (count($options) > 0) {
            ...
            if (! empty($item['storeids'])) {
                $stores = pdo_fetchall('select id,storename from ' . tablename('manor_shop_store') . ' where id in (' . $item['storeids'] . ' ) and uniacid=' . $_W['uniacid']);
            }
        ...
        $dispatch_data = pdo_fetchall('select * from ' . tablename('manor_shop_dispatch') . ' where uniacid=:uniacid and enabled=1 order by displayorder desc', array(
            ':uniacid' => $_W['uniacid']
        ));
        if ($_W['isajax'] && $_W['ispost']) {
            if ($diyform_plugin) {
                if ($_GPC['type'] == 1 && $_GPC['diyformtype'] == 2) {
                    show_json(-1,'替换模式只试用于虚拟物品类型，实体物品无效！请重新选择！');
                }
            }
            if (empty($_GPC['goodsname'])) {
                show_json(-1,'请输入商品名称！');
            }
            if (mb_strlen($_GPC['goodsname'], 'utf-8') > 11) {
                show_json(-1,'商品名称限制11个字！');
            }
            if (mb_strlen($_GPC['sub_title'], 'utf-8') > 11) {
                show_json(-1,'商品副标题名称限制11个字！');
            }
            ...
            $data = array(
                ...
                'ispackage' => intval($_GPC['ispackage']),
                ...
                'express_area' => trim($_GPC['express_area']),//配送区域
                'is_full_cat_power'=>intval($_GPC['is_full_cat_power']),
                'is_full_money'=>intval($_GPC['is_full_money']),
                'is_full_gift'=>intval($_GPC['is_full_gift']),
            );
            ...
            if (is_array($_GPC['cates'])) {
                ...
                foreach ($postcates as $pid) {
                    if ($cateset['catlevel'] == 3) {
                        $tcate = pdo_fetch('select id ,parentid from ' . tablename('manor_shop_category') . ' where id=:id and uniacid=:uniacid limit 1', array(
                            ':id' => $pid,
                            ':uniacid' => $_W['uniacid']
                        ));
                        $ccate = pdo_fetch('select id ,parentid from ' . tablename('manor_shop_category') . ' where id=:id and uniacid=:uniacid limit 1', array(
                            ':id' => $tcate['parentid'],
                            ':uniacid' => $_W['uniacid']
                        ));
                        $tcates[] = $tcate['id'];
            ...
            if (empty($id)) {
                pdo_insert('manor_shop_goods', $data);
                $id = pdo_insertid();
                plog('shop.goods.add', "添加商品 ID:{
                $id}
            ");
            } else {
                //pdo_delete('manor_shop_goods_for_city', array('goods_id'=>$id));
                unset($data['createtime']);
                pdo_update('manor_shop_goods', $data, array(
                    'id' => $id
                ));
                plog('shop.goods.edit', "编辑商品 ID:{
                $id}
            ");
            }
            //城市入库
            $city_arr = array_filter(array_unique(explode(';', $data['express_area'])));
            $db_old_city = pdo_getall('manor_shop_goods_for_city', array('goods_id'=>$id), 'city');
            if($db_old_city) {
                $old_city = array_column($db_old_city, 'city');
            }
            $goods_city = array_filter(array_unique($old_city));
            if($goods_city) {
                $add = array_diff($city_arr, $goods_city);
                $reduce = array_diff($goods_city, $city_arr);
            } else {
                $add = $city_arr;
            }
            if($add) {
                foreach($add as $item) {
                    ...
                    pdo_insert('manor_shop_goods_for_city', $city_insert_data);
                }
            }
            if($reduce) {
                foreach($reduce as $item) {
                    pdo_delete('manor_shop_goods_for_city', array('goods_id'=>$id, 'city'=>$item, 'uniacid' => $_W['uniacid']));
                }
            }
            $totalstocks = 0;
            ...
            for ($k = 0; $k < $len; $k ++) {
                ...
                if (! is_numeric($get_param_id)) {
                    pdo_insert('manor_shop_goods_param', $a);
                    $param_id = pdo_insertid();
                } else {
                    pdo_update('manor_shop_goods_param', $a, array(
                        'id' => $get_param_id
                    ));
            ...
            if (count($paramids) > 0) {
                pdo_query('delete from ' . tablename('manor_shop_goods_param') . " where goodsid = $id and id not in (" . implode(',', $paramids) . ')');
            } else {
                pdo_query('delete from ' . tablename('manor_shop_goods_param') . " where goodsid = $id");
            }
            $files = $_FILES;
            ...
            for ($k = 0; $k < $len; $k ++) {
                ...
                if (is_numeric($get_spec_id)) {
                    pdo_update('manor_shop_goods_spec', $a, array(
                        'id' => $get_spec_id
                    ));
                    $spec_id = $get_spec_id;
                } else {
                    pdo_insert('manor_shop_goods_spec', $a);
                ...
                for ($n = 0; $n < $itemlen; $n ++) {
                    ...
                    if (is_numeric($get_item_id)) {
                        pdo_update('manor_shop_goods_spec_item', $d, array(
                            'id' => $get_item_id
                        ));
                        $item_id = $get_item_id;
                    } else {
                        pdo_insert('manor_shop_goods_spec_item', $d);
                ...
                if (count($itemids) > 0) {
                    pdo_query('delete from ' . tablename('manor_shop_goods_spec_item') . " where uniacid ={$_W['uniacid']}
                and specid = $spec_id and id not in (" . implode(',', $itemids) . ')');
                } else {
                    pdo_query('delete from ' . tablename('manor_shop_goods_spec_item') . " where uniacid ={$_W['uniacid']}
                    and specid = $spec_id");
                }
                pdo_update('manor_shop_goods_spec', array(
            ...
            if (count($specids) > 0) {
                pdo_query('delete from ' . tablename('manor_shop_goods_spec') . " where uniacid ={$_W['uniacid']}
            ...
            for ($k = 0; $k < $len; $k ++) {
                ...
                if (empty($get_option_id)) {
                    pdo_insert('manor_shop_goods_option', $a);
                    $option_id = pdo_insertid();
                } else {
                    pdo_update('manor_shop_goods_option', $a, array(
                        'id' => $get_option_id
                    ));
            ...
            if (count($optionids) > 0) {
                pdo_query('delete from ' . tablename('manor_shop_goods_option') . " where goodsid = $id and id not in (" . implode(',', $optionids) . ')');
            } else {
                pdo_query('delete from ' . tablename('manor_shop_goods_option') . " where goodsid = $id");
            }
            if ($data['type'] == 3 && $pv) {
        ...
        if ($pv) {
            $virtual_types = pdo_fetchall('select * from ' . tablename('manor_shop_virtual_type') . ' where uniacid=:uniacid order by id asc', array(
                ':uniacid' => $_W['uniacid']
            ));
        }
        $levels = m('member')->getLevels();
        $details = pdo_fetchall('select detail_logo,detail_shopname,detail_btntext1, detail_btnurl1 ,detail_btntext2,detail_btnurl2,detail_totaltitle from ' . tablename('manor_shop_goods') . ' where uniacid=:uniacid and detail_shopname<>\'\'', array(
            ':uniacid' => $_W['uniacid']
        ));
        ...
        $sql = 'SELECT COUNT(*) FROM ' . tablename('manor_shop_goods') . $condition;
        $total = pdo_fetchcolumn($sql, $params);
        if (! empty($total)) {
            $sql = 'SELECT * FROM ' . tablename('manor_shop_goods') . $condition . ' ORDER BY `status` DESC, `displayorder` DESC,
        ...
        $id = intval($_GPC['id']);
        $row = pdo_fetch('SELECT id, title, thumb FROM ' . tablename('manor_shop_goods') . ' WHERE id = :id', array(
            ':id' => $id
        ));
        if (empty($row)) {
        ...
        pdo_delete('manor_shop_goods_for_city', array('goods_id'=>$id));
        plog('shop.goods.delete', "删除商品 ID:{$id}
                            标题:{$row['title']}
                            ");
        ...
        if (in_array($type, array(
            ...
            plog('shop.goods.edit', "修改商品上下架状态 ID:{
                                $id}
                            ");
            die(json_encode(array(
                'result' => 1,
        ...
        if (in_array($type, array(
            ...
            plog('shop.goods.edit', "修改商品类型 ID:{
                                $id}
                            ");
            die(json_encode(array(
                'result' => 1,
