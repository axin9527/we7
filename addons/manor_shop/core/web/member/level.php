...
if ($operation == 'display') {
	...
	$others = pdo_fetchall('SELECT * FROM ' . tablename('manor_shop_member_level') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY level asc");
	...
	if ($id == 'default') {
		...
		$level = pdo_fetch('SELECT * FROM ' . tablename('manor_shop_member_level') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => intval($id)));
	}
	$level_array = array();
	for ($i = 0; $i < 101; $i++) {
		$level_array[$i] = $i;
	...
	if (checksubmit('submit')) {
		...
		if (!empty($id)) {
			if ($id == 'default') {
				...
				pdo_update('manor_shop_member_level', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				plog('member.level.edit', "修改会员等级 ID: {$id}" . $updatecontent);
			}
		} else {
			pdo_insert('manor_shop_member_level', $data);
			$id = pdo_insertid();
	...
	$id = intval($_GPC['id']);
	$level = pdo_fetch('SELECT id,levelname FROM ' . tablename('manor_shop_member_level') . " WHERE id = '$id'");
	if (empty($level)) {
		message('抱歉，等级不存在或是已经被删除！', $this->createWebUrl('member/level', array('op' => 'display')), 'error');
	}
	pdo_delete('manor_shop_member_level', array('id' => $id, 'uniacid' => $_W['uniacid']));
