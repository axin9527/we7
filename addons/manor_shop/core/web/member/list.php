...
if ($op == 'display'){
    ...
    $sql = 'select dm.*,l.levelname,g.groupname,a.nickname as agentnickname,a.avatar as agentavatar from ' . tablename('manor_shop_member') . ' dm ' . ' left join ' . tablename('manor_shop_member_group') . ' g on dm.groupid=g.id' . ' left join ' . tablename('manor_shop_member') . ' a on a.id=dm.agentid' . ' left join ' . tablename('manor_shop_member_level') . ' l on dm.level =l.id' . ' left join ' . tablename('mc_mapping_fans') . "f on f.openid=dm.openid  and f.uniacid={$_W['uniacid']}" . " where 1 {$condition}  ORDER BY dm.id DESC";
    ...
    foreach ($list as & $row){
        ...
        $row['ordercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
        $row['ordermoney'] = pdo_fetchcolumn('select sum(goodsprice) from ' . tablename('manor_shop_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
    ...
    $total = pdo_fetchcolumn('select count(*) from' . tablename('manor_shop_member') . ' dm ' . ' left join ' . tablename('manor_shop_member_group') . ' g on dm.groupid=g.id' . ' left join ' . tablename('manor_shop_member_level') . ' l on dm.level =l.id' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid' . " where 1 {$condition} ", $params);
    ...
    if (checksubmit('submit')){
        ...
        pdo_update('manor_shop_member', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
        ...
        if ($hascommission){
            if (cv('commission.agent.changeagent')){
                ...
                if (!empty($adata)){
                    ...
                    pdo_update('manor_shop_member', $adata, array('id' => $id, 'uniacid' => $_W['uniacid']));
                    if (empty($_GPC['oldstatus']) && $adata['status'] == 1){
    ...
    $member['self_ordercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
    $member['self_ordermoney'] = pdo_fetchcolumn('select sum(goodsprice) from ' . tablename('manor_shop_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
    if (!empty($member['agentid'])){
    ...
    $member = pdo_fetch('select * from ' . tablename('manor_shop_member') . ' where uniacid=:uniacid and id=:id limit 1 ', array(':uniacid' => $_W['uniacid'], ':id' => $id));
    if (empty($member)){
        message('会员不存在，无法删除!', $this -> createWebUrl('member/list'), 'error');
    }
    if (p('commission')){
        $agentcount = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_member') . ' where  uniacid=:uniacid and agentid=:agentid limit 1 ', array(':uniacid' => $_W['uniacid'], ':agentid' => $id));
        if ($agentcount > 0){
            message('此会员有下线存在，无法删除! ', '', 'error');
        }
    }
    pdo_delete('manor_shop_member', array('id' => $_GPC['id']));
    ...
    if(!empty($black)){
        pdo_update('manor_shop_member', array('isblack' => 1), array('id' => $_GPC['id']));
        ...
        pdo_update('manor_shop_member', array('isblack' => 0), array('id' => $_GPC['id']));
...
} else if ($operation == 'setagent') {
    ca('member.member.setagent');
    $id = intval($_GPC['id']);
    $member = pdo_fetch('select * from ' . tablename('manor_shop_member') . ' where uniacid=:uniacid and id=:id limit 1 ', array(':uniacid' => $_W['uniacid'], ':id' => $id));
    if (empty($member)){
        message('会员不存在，无法设置分销!', $this -> createWebUrl('member/list'), 'error');
    }
    $agent = intval($_GPC['agent']);
    if(!empty($agent)){
        pdo_update('manor_shop_member',
            array(
                'isagent' => 1,
                'status'  => 1,
                'agenttime' => time()
            ...
            array('id' => $_GPC['id']));
        plog('member.member.agent', "设置分销 <br/>用户信息:  ID: {$member['id']} /  {$member['openid']}/{$member['nickname']}/{$member['realname']}/{$member['mobile']}") ;
        message('设置分销成功！', $this -> createWebUrl('member/list'), 'success');
    }else{
        pdo_update('manor_shop_member',
            array(
                'isagent' => 0,
            ...
            array('id' => $_GPC['id']));
        plog('member.member.agent', "取消分销 <br/>用户信息:  ID: {$member['id']} /  {$member['openid']}/{$member['nickname']}/{$member['realname']}/{$member['mobile']}") ;
        message('取消分销成功！', $this -> createWebUrl('member/list'), 'success');
    }
}
load() -> func('tpl');
