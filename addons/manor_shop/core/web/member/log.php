...
if ($op == 'display') {
    ...
    $total = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_member_log') . ' log ' . ' left join ' . tablename('manor_shop_member') . ' m on m.openid=log.openid' . ' left join ' . tablename('manor_shop_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('manor_shop_member_level') . ' l on m.level =l.id' . " where 1 {$condition} ", $params);
