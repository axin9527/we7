...
if ($operation == 'display' && $_W['isajax']){
    ...
    pdo_delete('manor_shop_member_log', array('openid' => $openid, 'status' => 0, 'type' => 0, 'uniacid' => $_W['uniacid']));
    ...
    if($log['money'] <= 0){
        pdo_update('manor_shop_member_log', array('money' => $money, 'couponid' => $couponid), array('id' => $log['id']));
    }
    ...
    $log = pdo_fetch('SELECT * FROM ' . tablename('manor_shop_member_log') . ' WHERE `logno`=:logno and `uniacid`=:uniacid limit 1', array(':uniacid' => $uniacid, ':logno' => $logno));
    if (!empty($log) && empty($log['status'])){
        pdo_update('manor_shop_member_log', array('status' => 1, 'rechargetype' => 'alipay'), array('id' => $log['id']));
