...
if($operation == 'category'){
    ...
    $city = trim($_GPC['city']);
    $goods = m('goods') -> getList(array('pagesize' => 100000, 'keywords' => trim($_GPC['keywords']), 'city'=>$city));
    ...
    $sql = 'SELECT id,nickname,headimgurl,level,content,createtime, images,append_images,append_content,reply_images,reply_content,append_reply_images,append_reply_content ' . ' FROM ' . tablename('manor_shop_order_comment') . ' where 1 ' . $condition . ' ORDER BY `id` DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
	...
	$url="http://api.map.baidu.com/geocoder?location=".$_GPC['latitude'].",".$_GPC['longitude']."&output=json&key=W8q2ZScUfUKAVe5xnfEGcI1W";
