...
$goods = pdo_fetch("SELECT * FROM " . tablename('manor_shop_goods') . " WHERE id = :id limit 1", array(':id' => $goodsid));
	if($goods['productprice'] > $goods['marketprice']) {
		$goods['reduce_price'] = $goods['productprice'] - $goods['marketprice'];
	} else {
		$goods['reduce_price'] = 0;
	}
...
if ($diyform_plugin){
    ...
    if ($_W['isajax'] && $_GPC['op'] == 'create'){
        ...
        if (empty($goods_temp)){
            pdo_insert('manor_shop_diyform_temp', $insert);
            $gdid = pdo_insertid();
        }else{
            pdo_update('manor_shop_diyform_temp', $insert, array('id' => $goods_temp['id']));
            $gdid = $goods_temp['id'];
        }
...
if ($_W['isajax']){
    ...
    if ($goods['usermaxbuy'] > 0){
        $order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('manor_shop_order_goods') . ' og ' . ' left join ' . tablename('manor_shop_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid ', array(':goodsid' => $goodsid, ':uniacid' => $uniacid, ':openid' => $openid));
	...
	$pics = unserialize($goods['thumb_url']);
    ...
    if (!empty($goods['hasoption'])){
        ...
        foreach ($allspecs as & $s){
            $items = pdo_fetchall("select * from " . tablename('manor_shop_goods_spec_item') . " where  `show`=1 and specid=:specid order by displayorder asc", array(":specid" => $s['id']));
    ...
    $specs = $allspecs;
    $params = pdo_fetchall("SELECT * FROM " . tablename('manor_shop_goods_param') . " WHERE uniacid=:uniacid and goodsid=:goodsid order by displayorder asc", array(':uniacid' => $uniacid, ":goodsid" => $goods['id']));
    $fcount = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_member_favorite') . ' where uniacid=:uniacid and openid=:openid and goodsid=:goodsid and deleted=0 ', array(':uniacid' => $uniacid, ':openid' => $openid, ':goodsid' => $goods['id']));
    pdo_query('update ' . tablename('manor_shop_goods') . " set viewcount=viewcount+1 where id=:id and uniacid='{$uniacid}' ", array(":id" => $goodsid));
    $history = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_member_history') . ' where goodsid=:goodsid and uniacid=:uniacid and openid=:openid and deleted=0 limit 1' , array(':goodsid' => $goodsid, ':uniacid' => $uniacid, ':openid' => $openid));
    if ($history <= 0){
        $history = array('uniacid' => $uniacid, 'openid' => $openid, 'goodsid' => $goodsid, 'deleted' => 0, 'createtime' => time());
        pdo_insert('manor_shop_member_history', $history);
    }
    ...
    $ret = array('goods' => $goods, 'followed' => $followed ? 1 : 0, 'followurl' => $followurl, 'followtip' => $followtip, 'saleset' => $saleset, 'pics' => $pics, 'options' => $options, 'specs' => $specs, 'params' => $params, 'commission' => $opencommission, 'commission_text' => $commission_text, 'level' => $level, 'shop' => $shop, 'goodscount' => pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_goods') . ' where uniacid=:uniacid and status=1 and deleted=0 ', array(':uniacid' => $uniacid)), 'cartcount' => pdo_fetchcolumn('select sum(total) from ' . tablename('manor_shop_member_cart') . ' where uniacid=:uniacid and openid=:openid and deleted=0 ', array(':uniacid' => $uniacid, ':openid' => $openid)), 'isfavorite' => $fcount > 0, 'stores' => $stores);
	$comment_num = pdo_fetch('select count("id") as num from '.tablename('manor_shop_order_comment').' where goodsid=:goodsid and uniacid=:uniacid and deleted !=1', array(':goodsid'=>$goods['id'], ':uniacid'=>$uniacid));
	$ret['comment_num'] = $comment_num['num'];
...
$this -> setHeader();
// 设置默认分享
$_W['shopshare']['desc'] = '吃水果，唐盛鲜果！产地直采，新鲜到手。'. (mb_strlen(trim($goods['description'])) > 0?trim($goods['description']):$goods['title']).'。非常不错，推荐你看一下。';
