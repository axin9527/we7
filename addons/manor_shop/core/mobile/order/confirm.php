...
$g_num  = count(array_unique(explode(',', $_GPC['cartids'])));
$pv = p('virtual');
...
if ($operation == 'display' || $operation == 'create'){
    ...
    if ($diyform_plugin){
        if (!empty($id)){
            $sql = 'SELECT id as goodsid,type,diyformtype,diyformid,diymode FROM ' . tablename('manor_shop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
...
if ($_W['isajax']){
    if ($operation == 'display'){
        ...
        $sql = 'select * from ' . tablename('manor_shop_coupon_data') . ' d';
        $sql .= ' inner join ' . tablename('manor_shop_coupon') . ' c on d.couponid = c.id';
        $sql .= " where d.openid=:openid and d.uniacid=:uniacid and  d.id=:dataid";
        if($_GPC['coupondataid']==""){
           $coupons="";
        }else{
            $coupons=pdo_fetch($sql, array(':openid' => $openid, ':uniacid' => $_W['uniacid'],':dataid'=>$_GPC['coupondataid']));
        ...
        if (empty($id)){
            ...
            $sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.issendfree,g.isnodiscount,g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid,g.storeids,g.isverify,g.deduct,g.manydeduct,g.virtual,o.virtual as optionvirtual,discounts,g.deduct2,g.ednum,g.edmoney,g.edareas,g.diyformtype,g.diyformid,diymode,g.dispatchtype,g.dispatchid,g.dispatchprice,g.express_area,g.is_full_cat_power,g.is_full_money,g.is_full_gift FROM ' . tablename('manor_shop_member_cart') . ' c ' . ' left join ' . tablename('manor_shop_goods') . ' g on c.goodsid = g.id ' . ' left join ' . tablename('manor_shop_goods_option') . ' o on c.optionid = o.id ' . " where c.openid=:openid and  c.deleted=0 and c.uniacid=:uniacid {$condition} order by c.id desc";
            $goods = pdo_fetchall($sql, array(':uniacid' => $uniacid, ':openid' => $openid));
            if (empty($goods)){
                show_json(-1, array('url' => $this -> createMobileUrl('shop/index')));
            }else{
            ...
            $sql = 'SELECT id as goodsid,type,title,weight,issendfree,isnodiscount, thumb,marketprice,storeids,isverify,deduct,manydeduct,maxbuy,usermaxbuy,discounts,total as stock,deduct2,ednum,edmoney,edareas,diyformtype,diyformid,diymode,dispatchtype,dispatchid,dispatchprice,express_area,is_full_cat_power,is_full_money,is_full_gift  FROM ' . tablename('manor_shop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
	        $data = pdo_fetch($sql, array(':uniacid' => $uniacid, ':id' => $id));
            $data['total'] = $total;
            $data['optionid'] = $optionid;
            if (!empty($optionid)){
                $option = pdo_fetch('select id,title,marketprice,goodssn,productsn,`virtual`,stock,weight from ' . tablename('manor_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $id, ':id' => $optionid));
                if (!empty($option)){
                    $data['optionid'] = $optionid;
            ...
            if ($data['usermaxbuy'] > 0){
                $order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('manor_shop_order_goods') . ' og ' . ' left join ' . tablename('manor_shop_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid ', array(':goodsid' => $data['goodsid'], ':uniacid' => $uniacid, ':openid' => $openid));
        ...
        $sale_plugin = p('sale');
	    $fixed_price_full_money = $fixed_price_full_gift = $fixed_price_full_cat_power = 0;
        $saleset = false;
        if ($sale_plugin){
            $saleset = $sale_plugin -> getSet();
            $saleset['enoughs'] = $sale_plugin -> getEnoughs();
        }
        $carrier_list = pdo_fetchall('select * from ' . tablename('manor_shop_store') . ' where  uniacid=:uniacid and status=1 and type in(1,3)', array(':uniacid' => $_W['uniacid']));
        ...
        foreach ($goods as & $g){
	        ...
	        if($g['is_full_money'] == 0 && $saleset['enoughs_reduce_power'] == 1) {
		        $fixed_price_full_money += $gprice;
	        }
	        //满邮费
	        if($g['is_full_cat_power'] == 0  && $saleset['enoughfree'] == 1) {
		        $fixed_price_full_cat_power += $gprice;
	        }
	        //满赠
	        if($g['is_full_gift'] == 0 && $saleset['enoughs_goods_power'] == 1) {
		        $fixed_price_full_gift += $gprice;
	        }
	        if($discountprice >=0) {
		        $fixed_price_full_money -= $discountprice;
		        $fixed_price_full_cat_power -= $discountprice;
		        $fixed_price_full_gift -= $discountprice;
	        }
	        $g['ggprice'] = $price;
            $realprice += $price;
            $goodsprice += $gprice;
        ...
        if ($isverify){
            ...
            if (empty($storeids)){
                $stores = pdo_fetchall('select * from ' . tablename('manor_shop_store') . ' where  uniacid=:uniacid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid']));
            }else{
                $stores = pdo_fetchall('select * from ' . tablename('manor_shop_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid']));
            }
        }else{
            $address = pdo_fetch('select id,realname,mobile,address,province,city,area,type,isdefault from ' . tablename('manor_shop_member_address') . ' where openid=:openid and deleted=0 and isdefault=1  and uniacid=:uniacid limit 1' , array(':uniacid' => $uniacid, ':openid' => $openid));
            if (!empty($carrier_list)){
                $carrier = $carrier_list[0];
            }
        ...
        if ($saleset){
            if (!empty($saleset['enoughfree'])){
                if (floatval($saleset['enoughorder']) <= 0){
                    ...
                    if ($fixed_price_full_cat_power >= floatval($saleset['enoughorder'])){
            ...
            if($saleset['enoughs_reduce_power'] == 1) {
	            foreach($saleset['enoughs'] as $e) {
		            if($fixed_price_full_money >= floatval($e['enough']) && floatval($e['money']) > 0) {
        ...
        $hascoupon = false;
        if ($hascouponplugin){
	        $_goods_ids = array_column($goods, 'goodsid');
            $couponcount = $plugc -> consumeCouponCount($openid, $realprice,$_goods_ids);
	    ...
	    $marketing_price = array('full_cat_power'=>0, 'full_gift'=>0, 'full_money'=>0);
	    foreach($goods as $key=>$good) {
		    //不参与满额包邮 不参与满减 不参与满赠
		    if($good['is_full_cat_power'] == 0 && $saleset['enoughfree'] == 1) {
			    $marketing_price['full_cat_power'] += $good['ggprice'];
		    }
		    if($good['is_full_gift'] == 0 && $saleset['enoughs_goods_power'] == 1) {
			    $marketing_price['full_gift'] += $good['ggprice'];
		    }
		    if($good['is_full_money'] == 0  && $saleset['enoughs_reduce_power'] == 1) {
			    $marketing_price['full_money'] += $good['ggprice'];
		    }
	    }
	    $marketing = array('full_cat_money'=>0, 'full_reduce'=>0, 'full_gift'=>0, 'marketing_price'=>$marketing_price);
	    //判断满额赠送
	    if($saleset['enoughs_goods'] && $saleset['enoughs_goods_power'] == 1 && $marketing_price['full_gift']>0) {
		    foreach($saleset['enoughs_goods'] as $k=>$v) {
			    if($v['gift_good']) {
				    $gift_goods = pdo_get('manor_shop_goods', array('uniacid'=>$uniacid, 'id'=>$v['gift_good']), array('id','title','thumb', 'status', 'total','marketprice','is_full_gift'));
				    if($gift_goods && $gift_goods['total'] >0 && $gift_goods['status'] == 1) {
					    $gift_goods['thumb'] = tomedia($gift_goods['thumb']);
					    if($marketing_price['full_gift'] >= $v['enough_good']) {
						    if($v['enough_good'] >= $marketing['full_gift']['money']['enough_good']) {
							    $gift_goods['money'] = $v;
							    $marketing['full_gift'] = $gift_goods;
						    }
					    } else {
					    	if(empty($marketing['full_gift'])) {
							    $marketing['full_gift'] = array('need_money' => $v['enough_good'], 'merge_money' => number_format($v['enough_good'] - $marketing_price['full_gift'], 2), 'goods' => $gift_goods);
						    }
					    }
				    }
			    }
        ...
        $pay_type = compact('wechat', 'alipay', 'unionpay');
        show_json(1, array('member' => $member, 'deductcredit' => $deductcredit, 'deductmoney' => $deductmoney, 'deductcredit2' => $deductcredit2, 'saleset' => $saleset, 'goods' => $goods, 'weight' => $weight / $buytotal, 'set' => m('common')->getSysset('shop'), 'fromcart' => $fromcart, 'haslevel' => !empty($level['id']) && $level['discount'] > 0 && $level['discount'] < 10, 'total' => $total,'sendcity'=>$sendcity, 'dispatchprice' => number_format($dispatch_price, 2), 'totalprice' => number_format($totalprice, 2), 'goodsprice' => number_format($goodsprice, 2), 'discountprice' => number_format($discountprice, 2), 'discount' => $level['discount'], 'realprice' => number_format($realprice, 2), 'address' => $address, 'carrier' => $carrier, 'carrier_list' => $carrier_list, 'dispatch_list' => $dispatch_list, 'isverify' => $isverify, 'stores' => $stores, 'isvirtual' => $isvirtual, 'changenum' => $changenum, 'hascoupon' => $hascoupon, 'couponcount' => $couponcount,  'marketing'=>$marketing, 'pay_type'=>$pay_type,'coupons'=>$coupons));
    }
        ...
        $full_cat_power = floatval($_GPC['full_cat_power']);
        $full_gift = floatval($_GPC['full_gift']);
        $full_money = floatval($_GPC['full_money']);
        $dflag = $_GPC['dflag'];
        $hascoupon = false;
        $couponcount = 0;
        $pc = p('coupon');
        if ($pc){
            ...
            if (empty($pset['closemember'])){
                $_goods_str = explode('|', $_GPC['goods']);
                $_goods_id_post = array();
                foreach ($_goods_str as $item) {
                    $_goods_arr = explode(',', $item);
                    $_goods_id_post[] = $_goods_arr[0];
                }
                $couponcount = $pc -> consumeCouponCount($openid, $totalprice, array_filter($_goods_id_post));
                $hascoupon = $couponcount > 0;
            }
        }
        $addressid = intval($_GPC['addressid']);
        $address = pdo_fetch('select id,realname,mobile,address,province,city,area from ' . tablename('manor_shop_member_address') . ' where  id=:id and openid=:openid and uniacid=:uniacid limit 1' , array(':uniacid' => $uniacid, ':openid' => $openid, ':id' => $addressid));
        $member = m('member') -> getMember($openid);
        $level = $level_info = m('member') -> getLevel($openid);
        $weight = $_GPC['weight'];
        $dispatch_price = 0;
        ...
        if (!empty($goods)){
            ...
            foreach ($goodsarr as & $g){
                ...
                $sql = 'SELECT id as goodsid,title,type, weight,total,issendfree,isnodiscount, thumb,marketprice,cash,isverify,goodssn,productsn,sales,istime,timestart,timeend,usermaxbuy,maxbuy,unit,buylevels,buygroups,deleted,status,deduct,manydeduct,`virtual`,discounts,deduct2,ednum,edmoney,edareas,diyformid,diyformtype,diymode,dispatchtype,dispatchid,dispatchprice,is_full_cat_power,is_full_money,is_full_gift FROM ' . tablename('manor_shop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
                ...
                if (!empty($optionid)){
                    $option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual`,weight from ' . tablename('manor_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $goodsid, ':id' => $optionid));
                    if (!empty($option)){
                        $data['optionid'] = $optionid;
                        $data['optiontitle'] = $option['title'];
                        $data['marketprice'] = $option['marketprice'];
            ...
            foreach ($allgoods as $g){
            	if($g['is_full_gift']==0 && $saleset['enoughs_goods_power'] == 1) {
		            foreach($saleset['enoughs_goods'] as $k => $v) {
						$gift_goods = pdo_get('manor_shop_goods', array('uniacid'=>$uniacid, 'id'=>$v['gift_good']), array('id','title','thumb', 'status', 'total','marketprice'));
						if($gift_goods && $gift_goods['total'] >0 && $gift_goods['status'] == 1) {
							$gift_goods['thumb'] = tomedia($gift_goods['thumb']);
							if($full_gift >= $v['enough_good']) {
								if($v['enough_good'] > $send_gift['money']['enough_good']) {
									$gift_goods['money'] = $v;
									$send_gift = $gift_goods;
								}
							} else {
								if(empty($send_gift)) {
									$send_gift = array('id' => '', 'need_money' => $v['enough_good'] - $full_gift, 'goods' => $gift_goods);
								}
							}
						}
		            }
            ...
            if ($sale_plugin){
                if ($saleset){
                    if (!$nowsendfree){
                        if (!empty($saleset['enoughfree'])){
                            if (floatval($saleset['enoughorder']) <= 0){
                                ...
                                if ($full_cat_power >= floatval($saleset['enoughorder'])){
                                    if (empty($saleset['enoughareas'])){
                                        $dispatch_price = 0;
                                    }else{
                                        $areas = explode(';', $saleset['enoughareas']);
                    ...
                    if($saleset['enoughs_reduce_power'] == 1) {
	                    foreach($saleset['enoughs'] as $e) {
		                    if($full_money >= floatval($e['enough']) && floatval($e['money']) > 0) {
        ...
        show_json(1, array('price' => $dispatch_price, 'hascoupon' => $hascoupon, 'couponcount' => $couponcount, 'deductenough_money' => $deductenough_money, 'deductenough_enough' => $deductenough_enough, 'deductcredit2' => $deductcredit2, 'deductcredit' => $deductcredit, 'deductmoney' => $deductmoney, 'send_gift'=>$send_gift));
    }
        ...
        if (!in_array($pay_type, array('weixin', 'alipay', 'unionpay'))){
            show_json(0, '未找到支付方式');
        }
        $address = false;
        ...
        $allgoods = array();
	    $give_goods = $_GPC['give_goods'];
	    $full_cat_power = $full_money = $full_gift = 0;
        $totalprice = 0;
        $goodsprice = 0;
        $weight = 0;
        $discountprice = 0;
	    if($give_goods) {
		    $goods .= $give_goods.',0,1|';
	    }
        $goodsarr = explode('|', $goods);
        $cash = 1;
        $level = $level_info = m('member') -> getLevel($openid);
        ...
        foreach ($goodsarr as $g){
            ...
            if ($_GPC['fromcart'] == 1){
                if ($diyform_plugin){
                    $cartdata = pdo_fetch('select id,diyformdataid,diyformfields,diyformdata from ' . tablename('manor_shop_member_cart') . ' ' . ' where goodsid=:goodsid and optionid=:optionid and openid=:openid and deleted=0 order by id desc limit 1' , array(':goodsid' => $data['goodsid'], ':optionid' => intval($data['optionid']), ':openid' => $openid));
                    if (!empty($cartdata)){
	        ...
	        $totalprice += $ggprice;
	        if($data['is_full_cat_power'] == 0 && $saleset['enoughfree'] == 1) {
		        $full_cat_power += $ggprice;
	        }
	        if($data['is_full_gift'] == 0 && $saleset['enoughs_goods_power'] == 1) {
		        $full_gift += $ggprice;
	        }
	        if($data['is_full_money'] == 0 && $saleset['enoughs_reduce_power'] == 1) {
		        $full_money += $ggprice;
	        }
	        if($g['is_full_money'] == 0 && $saleset['enoughs_reduce_power'] == 1) {}
            if ($data['isverify'] == 2){
        ...
        $deductenough = 0;
        if ($saleset && $saleset['enoughs_reduce_power'] == 1){
            foreach ($saleset['enoughs'] as $e){
                if ($full_money >= floatval($e['enough']) && floatval($e['money']) > 0){
                    $deductenough = floatval($e['money']);
                    if ($deductenough > $totalprice && $totalprice !=0){
            ...
            foreach ($allgoods as $g){
                ...
                if (!$sendfree){
                    if($g['dispatchtype'] == 1){
                        if ($g['dispatchprice'] > 0){
                            $dispatch_price += $g['dispatchprice'] * $g['total'];
	                        $dispatch_arr[$g['goodsid']] += $g['dispatchprice'] * $g['total'];
                        }
                    }else if($g['dispatchtype'] == 0){
                        if (empty($g['dispatchid'])){
                            $dispatch_data = m('order') -> getDefaultDispatch();
        ...
        if ($isverify){
            ...
            while (1){
                $count = pdo_fetchcolumn('select count(*) from ' . tablename('manor_shop_order') . ' where verifycode=:verifycode and uniacid=:uniacid limit 1', array(':verifycode' => $verifycode, ':uniacid' => $_W['uniacid']));
                if ($count <= 0){
                    break;
                }
                $verifycode = random(8, true);
        ...
        if($give_goods) {
        	$order['gift_good'] = $give_goods;
        }
	    $sql = 'SELECT id as goodsid,title,type, weight,total, thumb,marketprice,cash,dispatchprice FROM ' . tablename('manor_shop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
	    $give_goods_info = pdo_fetch($sql, array(':uniacid' => $uniacid, ':id' => $give_goods));
	     if(array_key_exists($give_goods, $dispatch_arr) ) {
		    $dispatch_price -= $give_goods_info['dispatchprice'];
		     $order['dispatchprice'] = $dispatch_price;
		     $order['price'] -= $give_goods_info['dispatchprice'];
		     $order['price'] -= $give_goods_info['marketprice'];
	    }
	    /*$order['goodsprice'] -= $give_goods_info['marketprice'];
	    if($order['goodsprice'] <=0) {
		    $order['goodsprice'] = 0;
	    }*/
	    $order['gift_good_info'] = json_encode($give_goods_info);
        $order['choose_pay_type'] = $pay_type;
        pdo_insert('manor_shop_order', $order);
        $orderid = pdo_insertid();
        if (is_array($carrier)){
            $up = array('realname' => $carrier['carrier_realname'], 'mobile' => $carrier['carrier_mobile']);
            pdo_update('manor_shop_member', $up, array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
            if (!empty($member['uid'])){
                load() -> model('mc');
        ...
        if ($_GPC['fromcart'] == 1){
            $cartids = $_GPC['cartids'];
            if (!empty($cartids)){
                pdo_query('update ' . tablename('manor_shop_member_cart') . ' set deleted=1 where id in (' . $cartids . ') and openid=:openid and uniacid=:uniacid ', array(':uniacid' => $uniacid, ':openid' => $openid));
            }else{
                pdo_query('update ' . tablename('manor_shop_member_cart') . ' set deleted=1 where openid=:openid and uniacid=:uniacid ', array(':uniacid' => $uniacid, ':openid' => $openid));
            }
        }

        foreach ($allgoods as $_goods){
            $order_goods = array('uniacid' => $uniacid, 'orderid' => $orderid, 'goodsid' => $_goods['goodsid'], 'price' => $_goods['marketprice']*$_goods['total'], 'total' => $_goods['total'], 'optionid' => $_goods['optionid'], 'createtime' => time(), 'optionname' => $_goods['optiontitle'], 'goodssn' => $_goods['goodssn'], 'productsn' => $_goods['productsn'], 'realprice' => $_goods['realprice'], 'oldprice' => $_goods['realprice'], 'openid' => $openid,);
            if($diyform_plugin){
                $order_goods['diyformid'] = $_goods['diyformid'];
                $order_goods['diyformdata'] = $_goods['diyformdata'];
                $order_goods['diyformfields'] = $_goods['diyformfields'];
            }
	        if($give_goods == $_goods['goodsid'] && $_goods['total'] == 1) {
		        $order_goods['is_give'] = 1;
	        } else {
		        $order_goods['is_give'] = 0;
	        }
	        pdo_insert('manor_shop_order_goods', $order_goods);
        }
        if ($deductcredit > 0){
        ...
        if (empty($virtualid)){
            ...
            if (isset($allgoods[0])){
                $vgoods = $allgoods[0];
                pdo_update('manor_shop_goods', array('sales' => $vgoods['sales'] + $vgoods['total']), array('id' => $vgoods['goodsid']));
            }
        }
...
if($_GPC['id'] && is_numeric($_GPC['id'])){
    $goods_t = pdo_get('manor_shop_goods', array('id'=>$_GPC['id']), array('isverify'));
}
if($goods_t && $goods_t['isverify'] ==2) {
    include $this -> template('order/mention');
} else {
