...
if ($operation == 'display' && $_W['isajax']){
    ...
    $plid = $log['plid'];
    if (empty($log)){
        $log = array('uniacid' => $uniacid, 'openid' => $member['uid'], 'module' => 'manor_shop', 'tid' => $order['ordersn'], 'fee' => $order['price'], 'status' => 0,);
    ...
    foreach ($order_goods as $data){
        ...
        if (!empty($data['optionid'])){
            $option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual` from ' . tablename('manor_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $data['goodsid'], ':id' => $data['optionid']));
            if (!empty($option)){
                if ($option['stock'] != -1){
    ...
    if ($type == 'weixin'){
        ...
        //用余额完全抵扣就不需要微信支付
        if($params['fee'] <=0) {
            pdo_update('manor_shop_order', array('paytype' => 21), array('id' => $order['id']));
            show_json(11);
        }
        load() -> model('payment');
        $setting = uni_setting($_W['uniacid'], array('payment'));
        ...
        pdo_update('manor_shop_order', array('paytype' => 22), array('id' => $order['id']));
        show_json(1);
    } elseif($type == 'unionpay') {
        pdo_update('manor_shop_order', array('paytype' => 23), array('id' => $order['id']));
        show_json(1);
    }
}
    ...
    $type = $_GPC['type'];
    if (!in_array($type, array('weixin', 'alipay', 'credit', 'cash', 'unionpay'))){
        show_json(0, '未找到支付方式');
    }
    $set = m('common')->getSysset(array('pay'));
	 if ($type=="credit" && $set['pay']['credit']==0){
      show_json(0, '余额支付功能已关闭');
    }
    ...
    //60 100,200
    $send_coupon_id = '';
    if($saleset && $saleset['enoughs_coupon_power'] == 1 && is_array($saleset['enoughs_coupon']) && count($saleset['enoughs_coupon'])>0) {
        $fan_coupon = array_filter(array_unique($saleset['enoughs_coupon']));
        ksort($fan_coupon);
        foreach($fan_coupon as $kk=>$cc) {
            if($log['fee'] >= $kk) {
                $send_coupon_id = $cc;
            }
        }
    ...
    if ($type == 'cash'){
        pdo_update('manor_shop_order', array('paytype' => 3), array('id' => $order['id']));
        $ret = array();
        $ret['result'] = 'success';
        $ret['type'] = 'cash';
        $ret['from'] = 'return';
        ...
        $ret['send_coupon_id'] = $send_coupon_id;
    ...
    if ($type == 'credit'){
        ...
        pdo_update('manor_shop_order', array('paytype' => 1), array('id' => $order['id']));
        $ret = array();
        $ret['result'] = 'success';
        $ret['type'] = $log['type'];
        $ret['from'] = 'return';
        ...
        $pay_result = $this -> payResult($ret);
        $pay_result['send_coupon_id'] = $send_coupon_id;
    ...
    $log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $uniacid, ':module' => 'manor_shop', ':tid' => $tid));
    if (empty($log)){
        die('支付出现错误，请重试!');
    }
    if ($log['status'] != 1){
    ...
    $orderid = pdo_fetchcolumn('select id from ' . tablename('manor_shop_order') . ' where ordersn=:ordersn and uniacid=:uniacid', array(':ordersn' => $log['tid'], ':uniacid' => $_W['uniacid']));
