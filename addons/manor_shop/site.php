 ...
 * 人人商城
 * 
 * @author Enjoy° QQ:834633039 
 */
if(!defined('IN_IA')) {
     exit('Access Denied');
}
require_once IA_ROOT. '/addons/manor_shop/version.php';
require_once IA_ROOT. '/addons/manor_shop/defines.php';
require_once manor_shop_INC.'functions.php'; 
require_once manor_shop_INC.'core.php';
require_once manor_shop_INC.'plugin/plugin.php';
require_once manor_shop_INC.'plugin/plugin_model.php';
class manor_shopModuleSite extends Core { 
    ...
    public function doWebPlugin(){   
        ...
        $p = $_GPC['p']; 
        $file = manor_shop_PLUGIN.$p."/web.php";
        if(!is_file($file)){ 
    ...
    public function doMobilePlugin(){ 
        ...
        $plugins = m('plugin')->getAll();
        $p = $_GPC['p'];
        $file = manor_shop_PLUGIN.$p."/mobile.php";
    ...
    public function doMobileActivity(){$this->_exec(__FUNCTION__, 'mid_autumn', false);}
    //订单
    ...
    public function getAuthSet() {
        global $_W;
        $set = pdo_fetch('select sets from ' . tablename('manor_shop_sysset') . ' order by id asc  limit 1');
