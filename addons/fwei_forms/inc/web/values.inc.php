...
if ( $act == 'detail') {
	...
	if (checksubmit('submit')) {
        ...
        $template = array(
            ...
            "url" => $_W['siteroot'] . 'app/' . $this->createMobileUrl('manor_shop'),
            "topcolor" => "#FF0000",
            "data" => array(
                'keyword1' => array(
                    'value' => urlencode('恭喜您,获得《免费送梨活动》,我们将稍后联系您'),
                    'color' => "#743A3A"
                ),
                'keyword2' => array(
                    'value' => urlencode('免费送梨活动'),
                    'color' => '#000000'
                ),
                'remark' => array(
                    'value' => urlencode('我们将在三个工作日内主动联系您,并将奖品快递给您,如有疑问请咨询平台客服'),
                    'color' => "#008000"
                )
            )
        );
        if($forms['template_id']) {
