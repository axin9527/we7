...
define('MB_ROOT', IA_ROOT . '/addons/fightgroups');
class Feng_fightgroupsModuleSite extends WeModuleSite {
	public function backlists(){
		...
		$frames = $backlist->getModuleFrames('fightgroups');
	...
	public function refund($orderno,$price,$type) {
		global $_GPC, $_W;
		include_once '../addons/fightgroups/source/WxPay.Api.php';
		...
		if ($result['return_code'] == 'SUCCESS') {
			...
			/*退款通知*/
			require_once IA_ROOT . '/addons/fightgroups/source/Message.class.php';
	...
	public function checkmode() {
		if (empty($this->module['config']['mode'])) {
			message('请先设置拼团模式', "../web/index.php?c=profile&a=module&do=setting&m=fightgroups", 'warning');
			exit;
		}
	}
