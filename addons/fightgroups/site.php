...
require IA_ROOT. '/addons/fightgroups/core/common/defines.php';
require TG_CORE . 'class/wlloader.class.php';
wl_load()->func('global');
wl_load()->func('pdo');
wl_load()->func('tpl');
wl_load()->func('message');
class FightgroupsModuleSite extends WeModuleSite {
	...
	public function payResult($params) {
		...
		if(empty($order_out['status'])){
			...
			$paytype = array('fee'=>0,'credit' => 1, 'wechat' => 2, 'alipay' => 2, 'delivery' => 3);
			...
			if ($params['result'] == 'success' && $params['from'] == 'notify') {
				...
				if(!$success){
					...
					if ($order_out['is_tuan'] == 0) {
						...
						/*更改库存*/
					if ($goodsInfo['gnum'] == 1) {
                        pdo_insert('log', array('cont'=>var_export(22, true)));
