...
//	wl_debug($settings);
	if (checksubmit('submit')) {
		load()->func('file');
        $r = mkdirs(IA_ROOT . '/attachment/fightgroups/cert/'. $_W['uniacid']);
		$r2 = mkdirs(TG_CERT.$_W['uniacid']);
		if(!empty($_GPC['cert'])) {
            $ret = file_put_contents(IA_ROOT . '/attachment/fightgroups/cert/'.$_W['uniacid'].'/apiclient_cert.pem', trim($_GPC['cert']));
            $ret2 = file_put_contents(TG_CERT.$_W['uniacid'].'/apiclient_cert.pem', trim($_GPC['cert']));
            $r = $r && $ret;
        }
        if(!empty($_GPC['key'])) {
            $ret = file_put_contents(IA_ROOT . '/attachment/fightgroups/cert/'.$_W['uniacid'].'/apiclient_key.pem', trim($_GPC['key']));
