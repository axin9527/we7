...
$ops = array('summary','received','detail','output','remark','address','confrimpay','confirmsend','cancelsend','refund', 'sendcoupon');
...
if ($op == 'detail') {
	...
	$table = tablename('manor_shop_coupon');
    $coupons = pdo_fetchall("select id, couponname from $table where uniacid = ${_W['uniacid']}");
...
if($op=='sendcoupon'){
    ...
    if ($coupon_id == ""){
        message('券必须要选择！', referer(), 'error');
    }
    $table = tablename('tg_order');
    $sql = "select openid, uniacid from $table where id = :id";
    $order = pdo_fetch($sql, array(':id' => $id));
    if ($order) {
        pdo_insert('manor_shop_coupon_data', array(
            "openid"   => $order['openid'],
            "uniacid"  => $order['uniacid'],
            "couponid" => $coupon_id,
            "gettime"  => time()
        ));
        pdo_update('manor_shop_coupon', array(
            "total" => "total - 1"
        ), array('id' => $coupon_id));
    }

    oplog('admin', "送订单", web_url('order/order/sendcoupon'), "$conpon_id");
    message('奖券发送成功', referer(), 'success');
