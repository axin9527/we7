<?php
	...
	if($op=='exchange'){
		...
		ORDER BY a.id DESC', array(':uniacid' => $_W['uniacid'], ':modu' => 'fightgroups', ':time' => TIMESTAMP,), 'couponid');
	}
	if($op=='main'){
		if(empty($_GPC['type'])){
			$type='djj';
...
//		}else
		if($type=='djj'){
			...
			DESC', array(':uniacid' => $_W['uniacid'], ':modu' => 'fightgroups', ':time' => TIMESTAMP), 'couponid');
			...
			foreach($djjs as$key=>$value){
				...
				array(':uniacid'=>$_W['uniacid'],':uid'=>$uid,':grantmodule'=>'fightgroups',':couponid'=>$value['couponid']));
				if ($djj_num) {
