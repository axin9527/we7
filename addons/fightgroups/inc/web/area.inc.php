...
if($operation=='limit'){
		...
		$areafile = IA_ROOT . "/addons/fightgroups/static/area/areaslimit";
		$areas = json_decode(@file_get_contents($areafile), true);
		if (!is_array($areas)) {
			require_once IA_ROOT . "/addons/fightgroups/static/area/xml2json.php";
			$file = IA_ROOT . "/addons/fightgroups/static/area/Area.xml";
		...
		if (checksubmit('submitlimit')) {
			...
			load()->func('file');
			$file_path = IA_ROOT . "/addons/fightgroups/arealimit/".$_W['uniacid'];
			if(!file_exists($file_path)){
				$r = mkdirs(IA_ROOT . "/addons/fightgroups/arealimit/".$_W['uniacid']);
				if($r){
//					$re = @fopen(IA_ROOT . "/addons/fightgroups/arealimit/".$_W['uniacid'].'/limitarea.xml',"x");
					$ret = file_put_contents(IA_ROOT . "/addons/fightgroups/arealimit/".$_W['uniacid'].'/limitarea.xml', '<?xml ?>');
				}
...
//				$ret = file_put_contents($file_path.'/arealimit.txt', "xx");
			...
			if(!empty($arraylimit)){
				require_once IA_ROOT . "/addons/fightgroups/source/areaxml.class.php";
		...
		if($yunfeiop=='display'){
			...
			$areafile = IA_ROOT . "/addons/fightgroups/static/area/areas";
			$areas = json_decode(@file_get_contents($areafile), true);
			if (!is_array($areas)) {
				require_once IA_ROOT . "/addons/fightgroups/static/area/xml2json.php";
				$file = IA_ROOT . "/addons/fightgroups/static/area/Area.xml";
