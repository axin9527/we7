...
class WxHongBaoHelper
{
    ...
    {
        define('PARTNERKEY', "0779E75BBA67B9B753A2FEDF85F208D4");
        try {
    ...
    */
    ...
    function curl_post_ssl($url, $vars, $second = 30, $aHeader = array(), $uniacid)
    {
        $ch = curl_init();
        //超时时间
        ...
        curl_setopt($ch, CURLOPT_SSLCERT, dirname(__FILE__) . DIRECTORY_SEPARATOR . 'zhengshu' . DIRECTORY_SEPARATOR . 'weixin_cert_file_' . $uniacid . '.pem');
        curl_setopt($ch, CURLOPT_SSLKEY, dirname(__FILE__) . DIRECTORY_SEPARATOR . 'zhengshu' . DIRECTORY_SEPARATOR . 'weixin_key_file_' . $uniacid . '.pem');
        curl_setopt($ch, CURLOPT_CAINFO, dirname(__FILE__) . DIRECTORY_SEPARATOR . 'zhengshu' . DIRECTORY_SEPARATOR . 'weixin_root_file_' . $uniacid . '.pem');
        if (count($aHeader) >= 1) {
