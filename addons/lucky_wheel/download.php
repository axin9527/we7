...
if($data=='mobileverify'){
    $statustitle='手机验证名单';	
	$list = pdo_fetchall("SELECT * FROM ".tablename('lucky_wheel_mobileverify')."  WHERE rid = :rid and uniacid=:uniacid ORDER BY id DESC" , array(':rid' => $rid,':uniacid'=>$_W['uniacid']));
	...
	$list = pdo_fetchall("SELECT * FROM ".tablename('stonefish_branch_doings')."  WHERE rid = :rid and uniacid=:uniacid and module=:module ".$where." ORDER BY id DESC" , array(':rid' => $rid,':uniacid'=>$_W['uniacid'],':module'=>'lucky_wheel'));
    //查询区域以及商家 以及中奖情况
	foreach ($list as &$row) {
		...
		$fid = pdo_fetchcolumn("SELECT from_user FROM " . tablename('lucky_wheel_fans') . "  WHERE mobile = :mobile and rid=:rid and uniacid=:uniacid", array(':mobile' => $row['mobile'],':rid' => $rid,':uniacid' => $_W['uniacid']));
		$row['awardinfo']='';
		$awards = pdo_fetchall("SELECT prizeid FROM " . tablename('lucky_wheel_fansaward') . " WHERE rid = :rid and from_user=:from_user", array(':rid' => $rid,':from_user' => $fid));
		if(!empty($awards)){
			foreach ($awards as &$awardid) {
				$row['prizename'] = pdo_fetchcolumn("SELECT prizename FROM " . tablename('lucky_wheel_prize') . "  WHERE id = :id", array(':id' => $awardid['prizeid']));
	...
	$list = pdo_fetchall("SELECT * FROM ".tablename('lucky_wheel_fans')."  WHERE rid = :rid and uniacid=:uniacid ".$where." ORDER BY id DESC" , array(':rid' => $rid,':uniacid'=>$_W['uniacid']));
	...
	$statustitle.='排名';
	$list = pdo_fetchall("SELECT * FROM ".tablename('lucky_wheel_fans')."  WHERE rid = :rid and uniacid=:uniacid ORDER BY ".$ORDER." DESC,id asc" , array(':rid' => $rid,':uniacid'=>$_W['uniacid']));
	...
	//导出标题    
    $list = pdo_fetchall("SELECT * FROM ".tablename('lucky_wheel_fansaward')." WHERE rid = :rid and uniacid=:uniacid ".$params." ORDER BY id DESC", array(':rid' => $rid,':uniacid'=>$_W['uniacid']));
    ...
    foreach ($list as $value) {
		...
		$prize = pdo_fetch("select prizerating,prizename from " . tablename('lucky_wheel_prize') . "  where id = :id", array(':id' =>$value['prizeid']));
	    ...
	    $html .= $value['status'] . "\t ,";	
	    $fans = pdo_fetch("select realname,mobile,qq,email,address,gender,telephone,idcard,company,occupation,position from " . tablename('lucky_wheel_fans') . "  where from_user = :from_user and rid = :rid and uniacid = :uniacid", array(':from_user' =>$value['from_user'],':rid' =>$rid,':uniacid' =>$_W['uniacid']));
		foreach ($ziduan as $ziduans) {
			if($exchange['is'.$ziduans]){
				if($ziduans=='gender'){
					if($fans[$ziduans]==0){
