...
class lucky_wheelModule extends WeModule
{
    public function fieldsFormDisplay($rid = 0)
    {
        ...
        $template  = pdo_fetchall("SELECT * FROM " . tablename('lucky_wheel_template') . " WHERE uniacid = :uniacid ORDER BY `id` asc", array(
            ':uniacid' => $uniacid
        ));
        if (empty($template)) {
        ...
        $tmplmsg = pdo_fetchall("SELECT * FROM " . tablename('lucky_wheel_tmplmsg') . " WHERE uniacid = :uniacid ORDER BY `id` asc", array(
            ':uniacid' => $uniacid
        ));
        if (!empty($rid)) {
            $reply    = pdo_fetch("SELECT * FROM " . tablename('lucky_wheel_reply') . " WHERE rid = :rid ORDER BY `id` desc", array(
                ':rid' => $rid
            ));
            $exchange = pdo_fetch("SELECT * FROM " . tablename('lucky_wheel_exchange') . " WHERE rid = :rid ORDER BY `id` desc", array(
                ':rid' => $rid
            ));
            $share    = pdo_fetchall("select * from " . tablename('lucky_wheel_share') . " where rid = :rid order by `id` desc", array(
                ':rid' => $rid
            ));
            $prize    = pdo_fetchall("select * from " . tablename('lucky_wheel_prize') . " where rid = :rid order by `id` asc", array(
                ':rid' => $rid
            ));
            if (!empty($reply)) {
        ...
        if (empty($share)) {
            ...
            foreach ($ids as $acid => $idlists) {
                $share[$acid] = array(
                    ...
                    "share_anniu" => "分享我的快乐",
                    "share_firend" => "我的亲友团",
                    "share_img" => "../addons/lucky_wheel/template/images/img_share.png",
                    "share_pic" => "../addons/lucky_wheel/template/images/share.png",
        ...
        if (!$reply) {
            $reply = array(
                "title" => "幸运大转盘活动开始了!",
                "start_picurl" => "../addons/lucky_wheel/template/images/activity-lottery-start.jpg",
                ...
                "end_picurl" => "../addons/lucky_wheel/template/images/activity-lottery-end.jpg"
            );
        }
        ...
        $reply['musicurl']              = empty($reply['musicurl']) ? "../addons/lucky_wheel/template/audio/bg.mp3" : $reply['musicurl'];
        ...
        $reply['bigwheelpic']           = empty($reply['bigwheelpic']) ? "../addons/lucky_wheel/template/images/activity-lottery-6.png" : $reply['bigwheelpic'];
        $reply['bigwheelimg']           = empty($reply['bigwheelimg']) ? "../addons/lucky_wheel/template/images/activity-inner.png" : $reply['bigwheelimg'];
        $reply['bigwheelimgan']         = empty($reply['bigwheelimgan']) ? "../addons/lucky_wheel/template/images/activity.png" : $reply['bigwheelimgan'];
        $reply['bigwheelimgbg']         = empty($reply['bigwheelimgbg']) ? "../addons/lucky_wheel/template/images/activity_bg.png" : $reply['bigwheelimgbg'];
    ...
    public function fieldsFormSubmit($rid)
    {
       ...
       // $token = @json_decode($content['content'], true);
        ...
        if ($token['config']) {
            if (empty($id)) {
                pdo_insert("lucky_wheel_reply", $insert);
                $id = pdo_insertid();
            } else {
                pdo_update('lucky_wheel_reply', $insert, array(
                    'id' => $id
                ));
            }
            if (empty($exchangeid)) {
                pdo_insert("lucky_wheel_exchange", $insertexchange);
            } else {
                pdo_update('lucky_wheel_exchange', $insertexchange, array(
                    'id' => $exchangeid
                ));
            }
        } else {
        ...
        foreach ($ids as $acid => $idlists) {
            ...
            if ($token['config']) {
                if (empty($_GPC['acid_' . $acid])) {
                    pdo_insert('lucky_wheel_share', $insertshare);
                } else {
                    pdo_update('lucky_wheel_share', $insertshare, array(
                        'id' => $_GPC['acid_' . $acid]
                    ));
                }
            }
        ...
        for ($i = 0; $i <= 11; $i++) {
            $insertprize = array(
                ...
                'break' => $_GPC['break'][$i],
                'password' => $_GPC['password'][$i],
            );
	        if($insertprize['prizetype'] == 'virtual') {
	        	$aid = intval($_GPC['aid'][$i]);
		        if(!$aid) {
		        ...
		        $insertprize['aid'] = $aid;
		        $insertprize['data'] = json_encode($coupon);
	        }
            if ($_GPC['turntable']) {
            ...
            if ($token['config']) {
                if (empty($_GPC['prize_id_' . $i])) {
                    pdo_insert('lucky_wheel_prize', $insertprize);
                } else {
                    pdo_update('lucky_wheel_prize', $insertprize, array(
                        'id' => $_GPC['prize_id_' . $i]
                    ));
                }
            }
        ...
        if ($updata['prize_num']) {
            pdo_update('lucky_wheel_reply', $updata, array(
                'id' => $id
            ));
        }
        if ($token['config']) {
    ...
    {
        global $_W;
        pdo_delete('lucky_wheel_reply', array(
            'rid' => $rid
        ));
        pdo_delete('lucky_wheel_exchange', array(
            'rid' => $rid
        ));
        pdo_delete('lucky_wheel_share', array(
            'rid' => $rid
        ));
        pdo_delete('lucky_wheel_prize', array(
            'rid' => $rid
        ));
        pdo_delete('lucky_wheel_prizemika', array(
            'rid' => $rid
        ));
        pdo_delete('lucky_wheel_fans', array(
            'rid' => $rid
        ));
        pdo_delete('lucky_wheel_fansaward', array(
            'rid' => $rid
        ));
        pdo_delete('lucky_wheel_fanstmplmsg', array(
            'rid' => $rid
        ));
        pdo_delete('lucky_wheel_sharedata', array(
            'rid' => $rid
        ));
        pdo_delete('lucky_wheel_mobileverify', array(
            'rid' => $rid
        ));
        return true;
    }
    public function settingsDisplay($settings)
    {
        ...
        if (checksubmit()) {
            ...
            $dat = array(
                ...
                'lucky_wheel_num' => $_GPC['lucky_wheel_num'],
                'lucky_wheel_oauth' => $_GPC['lucky_wheel_oauth'],
                'lucky_wheel_jssdk' => $_GPC['lucky_wheel_jssdk']
            );
