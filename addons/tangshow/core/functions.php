...
function m($name = '')
{
    ...
    $model = SHOP_CORE . "model/" . strtolower($name) . '.php';
    if (!is_file($model)) {
        die(' Model ' . $name . ' Not Found!');
    }
    require $model;
    $class_name      = 'Wei7Shop_' . ucfirst($name);
...
function p($name = '')
{
    if ($name != 'perm' && !IN_MOBILE) {
        static $_perm_model;
        if (!$_perm_model) {
            $perm_model_file = SHOP_PLUGIN . 'perm/model.php';
    ...
    $model = SHOP_PLUGIN . strtolower($name) . '/model.php';
    if (!is_file($model)) {
        return false;
    }
    require $model;
