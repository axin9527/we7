...
function web_url($segment, $params = array()) {
	...
	$url = $_W['siteroot'] . 'web/index.php?c=site&a=entry&m=alan_qrcode&';
	if(!empty($do)) {
		$url .= "do={$do}&";
	}
	if(!empty($ac)) {
...
function alan_log($message, $data = ''){
	if ($data) {
...
function alan_template($filename,$flag='') {
	global $_W;
	$name = 'alan_qrcode';
	if (defined('IN_SYS')) {
		$template = $_W['alansetting']['webview'];
		if (empty($template)) {
			$template = "default";
		} 
	...
	if (!is_file($compile) || filemtime($source) > filemtime($compile)) {
		alan_template_compile($source, $compile, true);
...
/********************app端自定义页面加载************************/
function alan_template_page($id, $flag = TEMPLATE_DISPLAY) {
	global $_W;
	$page = alan_page($id);
	if (empty($page)) {
	...
	$content = alan_template_parse($page['html']);
	file_put_contents($compile, $content);
	switch ($flag) {
		case TEMPLATE_DISPLAY:
		default:
			extract($GLOBALS, EXTR_SKIP);
			include alan_template('common/app_header');
			include $compile;
			include alan_template('common/app_footer');
			break;
		case TEMPLATE_FETCH:
			extract($GLOBALS, EXTR_SKIP);
			ob_clean();
...
/********************web端自定义页面加载************************/
function alan_tpl_wappage_editor($editorparams = '', $editormodules = array()) {
	global $_GPC;
	$content = '';
	load()->func('file');
	...
	if (!empty($filetree)) {
		foreach ($filetree as $file) {
			if (strexists($file, 'widget-')) {
				...
				$_GPC['iseditor'] = false;
				$display =  alan_template('wapeditor/'.$fileinfo['filename'], TEMPLATE_FETCH);
				$_GPC['iseditor'] = true;
				$editor =  alan_template('wapeditor/'.$fileinfo['filename'], TEMPLATE_FETCH);
...
/********************web端自定义页面加载************************/
function alan_tpl_ueditor($id, $value = '') {
	$s = '';
...
function alan_template_compile($from, $to, $inmodule = false) {
	$path = dirname($to);
	if (!is_dir($path)) {
		load()->func('file');
		mkdirs($path);
	}
	$content = alan_template_parse(file_get_contents($from), $inmodule);
...
function alan_template_parse($str, $inmodule = false) {
...
function alan_template_addquote($matchs) {
	$code = "<?php {$matchs[1]}?>";
...
function alan_message($msg, $redirect = '', $type = '') {
	global $_W;
	if($redirect == 'refresh') {
	...
	include alan_template('common/message', TEMPLATE_INCLUDEPATH);
	exit();
}

function alan_json($status = 1, $return = null) {
...
//		}
    ...
    function qrcode_logo($config, $path,$_wx_url, $is_merge=false){
        ...
        $logo = $config['qrcode_logo']=='../addons/alan_qrcode/template/images/110*110.png' ? realpath($config['qrcode_logo']) : realpath('../attachment/'.$config['qrcode_logo']);//上传至服务器的logo图片
        $QR = $path;//已经生成的原始二维码图
        if ($logo !== FALSE) {
        ...
        $save_path = '../attachment/'.ALAN_NAME.'/'.$p;
