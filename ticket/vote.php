...
function vote_range($limit) {
    $sql = "select * from ims_ticket_range order by vote_number desc limit $limit";
...
function vote_range_all() {
    $sql = "select * from ims_ticket_range order by vote_number desc";
    $result1 = pdo_fetchall($sql);
...
function my_friend($id, $start, $limit) {
    ...
    $sql = "select * from ims_ticket_user where ismock = 1 limit $number";
...
function vote_add($from, $to)
{
    ...
    if ($is_exists > 0) {
        $ret = pdo_query("update ims_ticket_range set vote_number = vote_number + 1 where user_id = :user_id",
            [
                "user_id" => $to,
            ]);
        if (!$ret) {
